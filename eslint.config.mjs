...
const browserGlobals = {
  ...
  HTMLElement: 'readonly', HTMLDetailsElement: 'readonly', Element: 'readonly', Node: 'readonly',
  ...
  // 媒体 / 图像 / DOM 遍历（2026-06-10 全项目审查补：语音录制、图片处理、i18n 文本遍历真实在用）
  MediaRecorder: 'readonly', Audio: 'readonly', AudioContext: 'readonly', OfflineAudioContext: 'readonly',
  Image: 'readonly', createImageBitmap: 'readonly', NodeFilter: 'readonly', DOMParser: 'readonly',
};
...
export default [
  ...
  {
    files: [
      'scripts/register-github.mjs', 'scripts/auto-create-ls-product.mjs', 'tests/e2e/**/*.mjs',
      // 2026-06-10 全项目审查补登记：这四个 Playwright 验证脚本同样在 page.evaluate 里用 DOM
      'scripts/noe-cognitive-runtime-verify.mjs', 'scripts/noe-cognitive-verify.mjs',
      'NOE_PHASE8_FUNCTIONAL_VERIFY.mjs', 'NOE_PHASE8_FUNCTIONAL_WALKTHROUGH.mjs',
    ],
    languageOptions: {
      ecmaVersion: 'latest',
      sourceType: 'module',
  ...
  // 5) 全局忽略：HTML（内嵌 script 无法用 JS parser）、构建产物、第三方
  {
    ignores: [
      ...
      '**/*.html',
      'eslint-report.json',
      'output/**',          // 审查/验证产物目录（含 BaiLongma vendor 源码快照），不是本项目代码
      'BaiLongma-audit/**',  // 同上：外部项目源码审查快照
      'benchmark/**',       // 模型评测一次性脚本（非产品代码，不在 lint files 范围）
      '.tmp-*',             // 临时脚本（跑完即删，防误提交触发 CI）
    ],
  },
];
