...
更新时间：2026-05-25
分析对象：Crow5 Desktop v1.3.13、本机安装路径 `/Applications/Crow5.app`、Noe 当前分支 `codex/paperclip-local-governance`
...
- Noe 当前代码：`server.js`、`src/**`、`public/app.js`、`docs/local-governance-learning-record.md`、测试结果。
...
对 Noe 的启发：现在 Noe 有很多强能力，但产品入口仍像“AI 控制台”。Crow5 更像“把想法推进为交付物的指挥台”。Noe 下一步应该把治理、委派、预算、审批包装成一个明确的 **Idea -> Plan -> Work -> Verify -> Archive** 流程。
...
结论：Crow5 的四大模块不是四个完全隔离产品，而是 **同一 Agent/Skill/模型底座在不同用户任务上的包装**。Noe 不应照抄四个大菜单，而应先把“开发”这条主路径做深，再用同一个底座扩展到“知识/办公/创作”。
...
这说明 Crow5 的迭代不是先堆新概念，而是优先修 **成本、性能、模型配置、多 Agent 体验、可见性**。Noe 当前已经做了预算治理和审批，这是对的；下一步要补的是“用量/成本和 Agent 工作状态在 UI 上一眼可见”。
...
重点不是名字，而是 **角色、标签、技能、权限四件事被配置化**。Noe 目前已有 room mode 和 role card，但还没有形成这种统一 registry。
...
Noe 当前只有轻量 `VectorIndex`：默认 128 维 hash embedding，可选 Ollama；还没有 AST、symbol graph、依赖图、增量索引、代码问答引用链。因此不能声称已经具备 Crow5 级“全工程理解”。
...
Noe 的 `SkillStore` 已经支持 `~/.noe-panel/skills/<name>/SKILL.md`、frontmatter、body 注入、数量/大小限制。这是基础，但比 Crow5 少：
...
Noe 当前有 `RoomAdaptersConfig`、Claude/Codex/Gemini/MiniMax/Ollama/OpenAI-compatible adapters，也有 `/v1/models` 兼容接口。差距在于：模型生态还停留在“开发者配置 adapter”，没有做到 Crow5 那种 **普通用户 provider 连接 + 模型市场 + 用量透明 + 自动匹配**。
...
本机 UI 没看到单独的 Ollama 入口，但 OpenCode/Crow5 的 provider 模式和自定义 provider 入口可以承接本地模型。Noe 现有 Ollama adapter 应保留，并在 Model Center 里把它包装成普通用户可理解的“本机模型”。
...
Noe 目前是 **Node/Express + Web UI + Electron packaging**。这不是错，但 Crow5 的 Tauri + sidecar 结构带来几个优势：
...
Noe 现在也有 store 和 route，但 registry 没有统一成产品概念。下一步应把 registry 作为一等架构，而不是继续按功能堆文件。
...
对 Noe 的落地要求：审批队列不能只管 shell 命令，还要管 **外部目录读写、密钥读取、Skill 调用、Plugin 执行、网络上传、模型密钥配置、自动接受权限作用域**。

## 三、个人与项目差距分析

这里的“你不会什么”不是评价个人能力，而是基于 Noe 当前代码尚未证明的能力清单。

### 3.1 技术能力差距清单（按优先级排序）

| 优先级 | 你当前还没有证明会什么 | 证据 | 为什么重要 |
| --- | --- | --- | --- |
| P0 | **配置驱动的多 Agent 调度系统** | Noe 有 `TaskGraph` 和 room dispatcher，但没有统一 `AgentRegistry + dispatchRules + AgentSkillBindings` | Crow5 的心脏就是角色/技能/权限/模型解耦 |
| P0 | **Agent 子任务会话化与结论汇总** | Noe 有委派和房间，但子 Agent 不是 Crow5 这种 session/message/part 级可追踪结构 | 不可追踪就无法审计、复盘、重试 |
| P0 | **真实代码库索引：AST + symbol graph + text search 融合** | Noe 仅有 hash/Ollama embedding，未见 Tree-sitter/LSP symbol 索引 | 全工程问答不能只靠向量 |
| P0 | **桌面权限模型与危险操作 guard** | Noe 有 owner-token、approval gate，但插件/skills/model/tool 权限没有统一策略 | 多 Agent 能写文件和跑命令时必须先有权限系统 |
| P1 | **Tauri/Rust sidecar 桌面架构** | Noe 仍以 Node/Electron 打包为主 | 不是必须迁移，但要理解 Crow5 的轻桌面边界 |
| P1 | **模型市场级 provider 管理** | Noe 有 adapter 配置，但没有 provider onboarding、模型缓存、用量额度 UI | 普通用户不会手写 adapter JSON |
| P1 | **Skill 市场与冲突治理** | Noe `SkillStore` 基础可用，但缺分类、绑定、权限、duplicate/rank | 技能多了以后，治理比加载更重要 |
| P1 | **前端工作台级复杂状态设计** | `public/app.js` 很大，UI 状态较集中 | Crow5 把会话、审查、文件、设置、provider 融在一个工作台 |
| P2 | **性能与成本诊断体系** | Noe 有 budget/metrics，但缺 KV cache 命中、prompt/token/schema 诊断 UI | 成本不可见时，多 Agent 很快失控 |
...
- **Agent 和 Skill 没有成为一等实体**：Crow5 的 Agent-Skill 绑定是产品功能；Noe 目前更像运行时配置。
- **项目/会话/消息/工具输出的系统记录不统一**：Crow5 用 SQLite schema 明确记录 session/message/part；Noe 的 room/session/metrics/audit 分散在多个 store。
...
- **你还不会把复杂能力压成一句主路径**：Noe 有治理、房间、插件、MCP、Autopilot，但用户第一眼不知道应该从哪里开始。
- **你还不会把多 Agent 结果做成可视化项目管理体验**：Crow5 的 Agent 头像、状态、审查、文件变化，让用户知道 AI 正在做什么；Noe 需要更统一的 Governance/Delegation/Agent Center。
- **你还不会把错误变成产品引导**：Crow5 把 provider、用量、权限、Git 缺失、文件变更都放进 UI；Noe 的很多错误还偏日志/API。
- **你还没有明确商业化路径的入口**：Crow5 的 provider、用量额度、模型管理直接服务付费与留存；Noe 的 license/payment 已有，但和核心工作流连接还弱。
...
- **迁移治理**：Crow5 启动时有数据库迁移流程和日志；Noe 的 SQLite/文件 store 需要明确 migration/versioning。
...
- **深入期实践项目：Agent Dispatcher + Codebase Index POC**。目标是输入一句任务，自动选择 Agent，并能回答 Noe 代码路径问题。
- **实践期实践项目：Noe Idea-to-Archive Flow**。目标是用户输入一句想法，系统生成计划、派发 Agent、记录审计、跑测试、归档报告。
...
- Day 4：写一个 Noe skill loader 对照表。
- Day 5：写 skill 权限策略草案：allow/ask/deny。
- Day 6：给 Noe 设计 `AgentSkillBinding` JSON。
- Day 7：用测试验证：禁用 skill 不会注入 prompt。

验收标准：Noe 有一个 `docs/agent-skill-registry-design.md`。
...
验收标准：Noe 有 provider registry 原型和测试。
...
验收标准：用户输入一句话，Noe 产生任务计划、执行记录、验证结果和归档报告。
...
目标：把 Noe 从“多功能面板”重构为“可追踪多 Agent 开发工作台”。
...
目标：让 Noe 有 Crow5 的扩展性，但保持本地优先和治理特色。
...
3. **明天画出 Noe 的新首页流程草图**：把首页从“功能入口集合”改成 `Idea -> Plan -> Work -> Verify -> Archive`。验收：一个前端开发者第一次打开 Noe，知道第一步该输入什么、系统会如何推进、失败在哪里看。
...
原因：Crow5 的优势不是某一个炫技点，而是 **Agent/Skill/Model/Permission/Session 的系统化组织**。Noe 先补这个组织层，收益最大、风险最低。
