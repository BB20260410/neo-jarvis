...
1. 顶栏点 📂 按钮打开配置 modal
2. 改 `rootPath`（默认 `~/Documents/noe-archive`）
3. 选目录结构：
...
```bash
curl -X PATCH http://localhost:51835/api/rooms/<roomId> \
...
```bash
# 切到项目 A
curl -X PUT http://localhost:51835/api/archive/config \
  -H 'Content-Type: application/json' \
...
# 切回默认
curl -X PUT http://localhost:51835/api/archive/config \
  -d '{ "rootPath": "~/Documents/noe-archive" }'
```
...
**Q：怎么验证归档真的工作？**
```bash
curl -X POST http://localhost:51835/api/archive/rooms/<roomId> | jq
