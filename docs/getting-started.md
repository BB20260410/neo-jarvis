...
1. 到 [GitHub Release](https://github.com/hxx-panel/noe/releases) 下载 `Noe.app.zip`
...
```bash
git clone https://github.com/hxx-panel/noe.git
cd noe
npm install
npm start    # 后端 51835
...
- **panel 启不起来**：`lsof -ti tcp:51835 | xargs kill`，再 `npm start`
- **MCP 连不上**：🔌 modal → 该 server → 🧪 测试连接看错误
- **某 AI 房卡住**：⏹ 立即结束 → 看错误 → ▶ 续跑
- **数据丢了**：rooms.json 自动 5MB 旋转，旧的在 ~/.noe-panel/rooms.json.bak
