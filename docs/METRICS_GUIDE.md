...
```js
const ws = new WebSocket('ws://localhost:51835/ws/global');
ws.onmessage = (e) => {
...
| 文件 | 内容 | 权限 |
|---|---|---|
| `~/.noe-panel/metrics-YYYY-MM.jsonl` | 一个月一份，每行一条 JSON | 0o600 |
| `~/.noe-panel/metrics-YYYY-MM.jsonl.<ts>` | 单文件超 50MB 时滚动归档 | 0o600 |
...
```bash
cat ~/.noe-panel/metrics-2026-05.jsonl | jq -s '.' > export-may.json
```
...
**Q：UI 不显示数据？**
A：1) 看 `/api/metrics/overview` 直接 curl 是否返数据；2) 看 `~/.noe-panel/metrics-*.jsonl` 是否有内容；3) panel 没重启过 → 老代码没装新埋点。
