...
- 威胁：本机其他 UID 进程 `curl` 拿 RCE / 读数据。Token 经 `getOrCreateOwnerToken` 存 `~/.noe-panel/owner-token.txt`。
...
- `src/security/uploadAllowlist.js`：`~/.noe-panel/upload-allowlist.json` 配置后，网络上传/webhook 仅允许列表内 host（在 SSRF 私网防护之上的额外收敛）。
