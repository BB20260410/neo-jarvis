...
> Noe v0.52 W2 起，任意 CLI 工具都能通过一份 JSON manifest 接入。
> 安装位置：`~/.noe-panel/cli-plugins/<id>.json`（0o600，原子写）
...
cp my-tool.json ~/.noe-panel/cli-plugins/my-tool.json
...
### Step 3：装 + 跑
```bash
cp ollama-cli.json ~/.noe-panel/cli-plugins/
