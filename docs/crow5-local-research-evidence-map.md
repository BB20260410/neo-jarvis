...
- Noe 的 skill loader 应保留体量上限、冲突检测、权限声明和来源解释，否则大技能集合会快速污染 prompt。

自检/审查：

- 只读 bundle 与用户目录中的 `SKILL.md` 元信息，没有复制技能正文。
- Noe 已经用自己的 `xike-*` profile 替代 Crow5 角色名，后续也应继续避免直接使用 Crow5 品牌角色。
...
- 方案均映射到 Noe 现有模块或明确的自研模块，没有要求复制 Crow5 私有实现。
...
- 对 Noe 已实现能力做差距矩阵：Crow5 pattern -> Xike module -> missing tests。
...
- 将 P0-P9 路线转成 Noe issue/backlog 或本地 task graph。
