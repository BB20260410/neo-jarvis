...
export function createEvolutionOutcome({
  ...
  function diff(before, after) {
    ...
    for (const p of Object.keys(after || {})) {
      ...
      files[p] = {
        ...
        codeLinesDelta: a.codeLines - (Number.isFinite(Number(b.codeLines)) ? Number(b.codeLines) : a.codeLines),
        linesDelta: a.lines - (Number.isFinite(Number(b.lines)) ? Number(b.lines) : a.lines),
      };
    }
    return files;
  }
  ...
  function record({ patchPlanId = '', before = {}, after = {}, applied = true } = {}) {
    const s = summarize({ before, after });
    const summary = { patchPlanId, at: now(), applied: applied !== false, ...s };
