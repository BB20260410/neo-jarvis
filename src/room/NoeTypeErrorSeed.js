...
export function createTypeErrorSeed(deps) {
  ...
  async function runOnce() {
    ...
    const targets = parseTypecheckTargets(output, { maxErrorsPerFile });
    ...
    const errList = target.errors.slice(0, 5).map((e) => `${e.line}:${e.code}`).join(', ');
    const objective = `修 ${target.file} 的 ${target.errorCount} 个结构性类型 error(${errList})：加 JSDoc 类型注解或修真实类型问题(属性不存在/null 误用)，禁用 @ts-ignore/@ts-nocheck/any 消音`;
    const goalId = goalSystem.add({
      title: `修 ${target.file} 的类型 error`.slice(0, 120),
      source: SIGNAL_SOURCE,
      why: 'type_error_fix 域(扩展自主能力域)：逐文件开启类型安全，修结构性 bug',
      steps: [{ step: objective.slice(0, 120), kind: 'think' }],
      meta: { signal: SIGNAL, file: target.file, targetFile: target.file, errorCount: target.errorCount, expectedVerdict: 'logic_changed', discoveredAt: now() },
    });
    if (!goalId) return { ok: false, reason: 'add_rejected' };
    return { ok: true, goalId, targetFile: target.file, errorCount: target.errorCount };
  }
