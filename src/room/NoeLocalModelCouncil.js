...
import { cleanVisibleModelText, redactSensitiveText } from '../runtime/NoeContextScrubber.js';
...
const SECRET_PATTERNS = [
  /(MINIMAX_API_KEY|OBSIDIAN_API_KEY|OPENAI_API_KEY|XIAOMI_API_KEY)\s*[:=]\s*["']?[^"'\s]+/gi,
  /(Authorization:\s*Bearer\s+)(?!\[redacted\])[A-Za-z0-9._~+/=-]{8,}/gi,
  /(X-Panel-Owner-Token["':\s]+)[0-9a-f]{32,}/gi,
  /sk-[A-Za-z0-9_-]{20,}/g,
  /tp-[a-z0-9]{20,}/gi,
  /\?t=[0-9a-f]{32,}/gi,
];
...
export function assertLocalCouncilLedgerSafe(ledger) {
  ...
  const unsafe = SECRET_PATTERNS.some((pattern) => new RegExp(pattern.source, pattern.flags.replace(/g/g, '')).test(text));
