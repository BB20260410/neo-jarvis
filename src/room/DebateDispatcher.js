  ...
  async _runRound(roomId, kind, adapterList, buildMessages, abortSignal, macroRound) {
    ...
    const promises = adapterList.map(async ({ member, adapter }) => {
      ...
      try {
        ...
        // 截断感知:发言被 length/max_tokens 截断时,半截提案/终稿不能当完整发言进 debate。
        // 标注 content + 标 incomplete,judge 合成与前端都能看见「这是半截」,不当完整结论。
        const incomplete = isIncompleteChatResult(result);
        const turn = {
          speaker: member.adapterId,
          displayName: member.displayName,
          content: incomplete ? markTruncatedReply(result.reply, result) : result.reply,
          tokensIn: result.tokensIn,
          tokensOut: result.tokensOut,
          ...(incomplete ? { incomplete: true, finishReason: truncationFinishReason(result) } : {}),
        };
