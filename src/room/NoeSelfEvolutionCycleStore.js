...
// progressBlocker 仅在传入 capability（与 trigger 一致的 autodrive 能力）时可信；
// 默认不传 → progressBlocker 会按「无 autodrive」报告，调用方不得把 store.computeStage 当健康态。
function computeStage(cycle = {}, capability = {}) {
  try {
    const maxReworkRounds = Number(capability.maxReworkRounds);
    const loop = evaluateNoeSelfEvolutionLoop({
      ...cycle,
      dryRun: true,
      hasConsensusAutodrive: capability.hasConsensusAutodrive === true,
      hasCompletionAutodrive: capability.hasCompletionAutodrive === true,
      reworkEnabled: capability.reworkEnabled === true,
      // Required for post_review_rework_ready (loop defaults maxReworkRounds to 0 → never rework).
      maxReworkRounds: Number.isFinite(maxReworkRounds) && maxReworkRounds > 0
        ? maxReworkRounds
        : (Number(cycle.maxReworkRounds) > 0 ? Number(cycle.maxReworkRounds) : 0),
    });
...
export class NoeSelfEvolutionCycleStore {
   ...
   * @param {boolean} [opts.hasConsensusAutodrive] optional; only needed if reading progressBlocker via evaluateLoop
   * @param {boolean} [opts.hasCompletionAutodrive]
   * @param {boolean} [opts.reworkEnabled]
   * @param {number} [opts.maxReworkRounds] must be >0 for post_review_rework_ready (server default 2)
   */
  constructor({
    projectId = 'noe',
    ...
    const maxR = Number(maxReworkRounds);
    this.capability = {
      hasConsensusAutodrive: hasConsensusAutodrive === true,
      hasCompletionAutodrive: hasCompletionAutodrive === true,
      reworkEnabled: reworkEnabled === true,
      maxReworkRounds: Number.isFinite(maxR) && maxR > 0 ? maxR : 0,
    };
  }

  /**
   * Full loop evaluate (includes progressBlocker). Prefer over raw evaluateNoeSelfEvolutionLoop({...cycle})
   * so autodrive capability matches store construction / production trigger.
   * @param {object} [cycle]
   */
  evaluateLoop(cycle = {}) {
    return evaluateNoeSelfEvolutionLoop({
      ...cycle,
      dryRun: true,
      hasConsensusAutodrive: this.capability.hasConsensusAutodrive,
      hasCompletionAutodrive: this.capability.hasCompletionAutodrive,
      reworkEnabled: this.capability.reworkEnabled,
      maxReworkRounds: this.capability.maxReworkRounds > 0
        ? this.capability.maxReworkRounds
        : (Number(cycle.maxReworkRounds) > 0 ? Number(cycle.maxReworkRounds) : 0),
    });
  }
  ...
  upsert(cycleInput = {}, opts = {}) {
    ...
    const stage = computeStage(cycle, this.capability);
    cycle.stage = stage;
