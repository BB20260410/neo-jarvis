 ...
 * @property {number} [lessonsConsidered]
 */
 ...
 * @property {boolean} bumpHits
 */

/**
 * @typedef {Object} RecallResult
 * @property {RejectLesson[]} lessons
 */

/**
 * @typedef {Object} Thresholds
 * @property {number} [diceThreshold]
 * @property {number} [minChars]
 */

/**
 * @typedef {Object} RecallFactoryOptions
 * @property {function(RecallOptions): RejectLesson[] | null} [recall]
 * @property {string} [projectId]
 * @property {function(): number | number} [now]
...
// 从 reject / verify lesson 的脱敏 summary 提取 objective。
//   支持：被复核拒绝（…）。 / 验证未绿（…）。
export function extractObjectiveFromSummary(body = '') {
  const text = String(body || '');
  const m = text.match(/被复核拒绝（([\s\S]+?)）。/)
    || text.match(/验证未绿（([\s\S]+?)）。/);
...
/**
 * Re-order open goals so near-duplicates of reject lessons sink to the end (or are filtered).
 * Prevents openSelfEvolutionGoals()[0] from permanently replaying a doomed objective already rejected.
 * Fail-open: if recall unavailable, returns original order.
 *
 * @param {Array<{id?:string,title?:string,goal?:string,objective?:string}>} goals
 * @param {(objective:string)=>ClassificationResult|null|undefined} recallRejectLessons
 * @param {{ demoteOnly?: boolean }} [opts] demoteOnly=true (default): keep goals but after non-similar; false: drop similar
 */
export function orderOpenGoalsAvoidingRejectLessons(goals, recallRejectLessons, opts = {}) {
  const demoteOnly = opts.demoteOnly !== false;
  const rows = Array.isArray(goals) ? goals.filter(Boolean) : [];
  if (typeof recallRejectLessons !== 'function' || !rows.length) {
    return {
      ordered: rows.slice(),
      demoted: [],
      blocked: [],
      recallUnavailable: typeof recallRejectLessons !== 'function',
    };
  }
  /** @type {Array<object>} */
  const nonSimilar = [];
  ...
  const demoted = [];
  for (const g of rows) {
    const objective = String(g.objective || g.title || g.goal || g.text || '').trim();
    let verdict = null;
    ...
    if (verdict && verdict.similar === true) {
      demoted.push({
        ...
        reason: verdict.reason || 'near_duplicate_rejected',
        lessonObjective: verdict.lessonObjective || '',
        goal: g,
      });
      similarGoals.push(g);
    } else {
  ...
  if (!demoteOnly) {
    return { ordered: nonSimilar, demoted, blocked: demoted, recallUnavailable: false };
  }
  return { ordered: [...nonSimilar, ...similarGoals], demoted, blocked: [], recallUnavailable: false };
}
