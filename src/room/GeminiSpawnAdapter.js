...
// 2026-06：Gemini 自动降级链——一个 model 配额满了自动试下一个
//   3.1 Pro Preview 是当前已验证可用的最强 CLI 默认；旧 Pro/Flash 作为后备。
//   顺序按"从高到低"；用户在 UI 指定 model 时把它放链首，其他作 fallback
export const GEMINI_FALLBACK_CHAIN = ['gemini-3.1-pro-preview', 'gemini-2.5-pro', 'gemini-2.5-flash', 'gemini-2.5-flash-lite'];
export const GEMINI_LIVE_PING_CHAIN = ['gemini-2.5-flash', 'gemini-2.5-flash-lite'];

export function normalizeGeminiPreferredModel(preferred) {
  const model = String(preferred || '').trim();
  if (!model) return '';
  // 3.1 Pro Preview 已在 gemini-cli 0.44.1 真实验证可用。
  // 用户界面里常填并不存在的 gemini-3.5-*；运行时映射到稳定 flash，避免每轮先卡一次无效模型。
  if (/^gemini-3\.5(?:[.-]|$)/i.test(model)) return 'gemini-2.5-flash';
  return model;
}

export function buildGeminiFallbackChain(preferred) {
  const normalized = normalizeGeminiPreferredModel(preferred);
  if (!normalized) return [...GEMINI_FALLBACK_CHAIN];
  const rest = GEMINI_FALLBACK_CHAIN.filter(m => m !== normalized);
  return [normalized, ...rest];
}

export function buildGeminiLivePingFallbackChain(preferred) {
  const normalized = normalizeGeminiPreferredModel(preferred);
  if (GEMINI_LIVE_PING_CHAIN.includes(normalized)) {
    return [normalized, ...GEMINI_LIVE_PING_CHAIN.filter(m => m !== normalized)];
  }
  return [...GEMINI_LIVE_PING_CHAIN];
}

/**
 * 清理 gemini CLI PTY 输出：ANSI/控制字符 + CLI 自身告警（Warning/Ripgrep/MCP 噪声），只留模型正文。
 * P1 去噪（2026-06-10 实测）：CLI 启动加载 MCP server 失败时把 "MCP issues detected. Run /mcp list
 * for status." **混进正文同一行**，按行过滤删不掉，需短语级剥除。
 */
export function cleanGeminiCliOutput(text) {
  return String(text || '')
    ...
    .replace(/^Warning:.*$/gm, '')
    .replace(/MCP issues detected\.\s*(?:Run \/mcp list for status\.?)?\s*/g, '')
    .trim();
}
...
export class GeminiSpawnAdapter extends RoomAdapter {
  constructor(opts = {}) {
    super({
      ...
      // 默认链首使用当前已验证最强 Gemini；配额满时自动降级到 2.5 Pro/Flash。
  ...
  getNativeCapabilities() {
    return normalizeNativeCapabilities({
      ...
      skills: [
        'Gemini 账号级模型能力',
        'Gemini CLI 本机 OAuth 会话',
        'Gemini CLI 自身上下文与工具能力',
      ],
      plugins: [
        'Gemini CLI 运行时可见的扩展能力',
      ],
      tools: [
        'Gemini CLI 原生工具链',
        'Gemini CLI 可用时的 Google Search/检索能力',
      ],
      mcp: [
        'Gemini CLI 自身支持的外部工具能力',
      ],
      notes: [
        '集群协同不会把 Gemini 当成 GPT/Claude 的公共 Skill 容器;Gemini 成员按 Gemini CLI 自己的运行时执行。',
        'Google Search/检索能力取决于当前 Gemini CLI 版本和账号配置是否实际开放。',
      ],
    });
  }
  ...
  async _doChat(messages, opts = {}) {
    ...
    const chain = opts.livePing
      ? buildGeminiLivePingFallbackChain(opts.model || this.model)
      : buildGeminiFallbackChain(opts.model || this.model);
    let lastErr = null;
  ...
  async _doChatOnce(prompt, model, opts = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      ptyProcess.onExit(({ exitCode }) => {
        ...
        let reply = cleanGeminiCliOutput(stdout);
        if (exitCode === 0 && reply) {
