...
import { PeerCritiqueGate } from './PeerCritiqueGate.js';
...
export class ArenaDispatcher {
  constructor({ store, adapters, broadcast, metrics }) {
    ...
    // AutoScientists 式同行评审门：提案产出后、昂贵 judge 联网核对前，剪掉弱/重复提案
    this.critiqueGate = new PeerCritiqueGate({ broadcast: this.broadcast });
  }
  ...
  async start(roomId, topic, _options = {}) {
    ...
    try {
      ...
      // ===== Phase 1.5：同行评审门（peer-critique-before-compute，AutoScientists 思想）=====
      // 昂贵的 judge 联网核对前，用便宜的本地评审员剪掉弱/重复提案，judge 只核对存活方案（省 token + 聚焦）。
      // fail-open：评审失败/降级一律保留全部，绝不卡住房间。可经 NOE_ARENA_CRITIQUE=0 关闭。
      let arenaProposals = validTurns;
      if (process.env.NOE_ARENA_CRITIQUE !== '0' && validTurns.length >= 3) {
        const critic = this.adapters.get('ollama') || this.adapters.get('lmstudio') || judge;
        const gate = await this.critiqueGate.evaluate({
          roomId, topic, critic, abortSignal: aborter.signal,
          proposals: validTurns.map(t => ({ anonId: t.anonId, content: t.content, speaker: t.speaker, displayName: t.displayName })),
        });
        if (!gate.skipped && Array.isArray(gate.survivors) && gate.survivors.length) {
          arenaProposals = gate.survivors;
          try {
            this.store.appendTurn(roomId, 'critique', {
              speaker: 'peer-critique-gate',
              displayName: '同行评审门',
              content: gate.degraded
                ? `（评审降级，保留全部 ${validTurns.length} 份提案）`
                : `存活 ${gate.survivors.map(p => p.anonId).join('/')}；剪除 ${gate.rejected.map(p => p.anonId).join('/') || '无'}`,
            });
          } catch { /* appendTurn 失败不阻断 */ }
        }
      }
      const anonymized = arenaProposals.map(t => `## 方案 ${t.anonId}\n\n${t.content}`).join('\n\n===\n\n');
