// ChatRoomStore — 房间数据结构 + 持久化（~/.noe-panel/rooms.json）
//
// 数据模型：
// room = {
...
import { readFileSync, writeFileSync, mkdirSync, existsSync, chmodSync, renameSync, copyFileSync, statSync, openSync, fsyncSync, closeSync } from 'node:fs';
...
const STORE_FILE = join(homedir(), '.noe-panel', 'rooms.json');
const ARCHIVE_FILE = join(homedir(), '.noe-panel', 'rooms-archive.json');  // v2.2 split:archived 房独立文件
const STORE_DIR = dirname(STORE_FILE);

// stable-sort replacer:JSON.stringify 按 key 字母序输出 object 内容,避免 Object.assign
// 改键顺序后 fingerprint 误判变化重写。null/array/原始类型直接 return value。
function stableSortReplacer(_key, value) {
...
export class ChatRoomStore {
  constructor() {
    ...
    // v2.2 split:active 与 archived 房分文件持久化,各自 fingerprint 比对决定是否写盘,
    // 避免 archived 房(主文件 34% 体积)在每次 active 房 turn 后被重写放大。
    this._lastActiveFingerprint = null;
    this._lastArchiveFingerprint = null;
    this.load();
  }
  ...
  load() {
    // v2.2 split:先加载主 active 文件,再(若存在)加载 archive 文件 merge 到同 Map
    this._loadFile(STORE_FILE, 'active');
    if (existsSync(ARCHIVE_FILE)) this._loadFile(ARCHIVE_FILE, 'archive');
    // S27 B3:主文件大小 audit + 旧 archived 警告(只在 load 末尾跑一次)
    try {
  ...
  _loadFile(filePath, kind) {
    try {
      ...
      if (rooms.length > 200) {
        console.warn(`[ChatRoomStore.load] ${kind} 含 ${rooms.length} 个房间，超过 200 上限，仅加载最新 200`);
      ...
      for (const r of rooms) {
        ...
        // 重启时把 running 改回 paused，避免房间假运行；taskList 内残留 running
        // 任务必须独立恢复。历史上出现过"房间已 paused，但 task 仍 running"，
        // 若只看 room.status 会漏掉，导致集群协同/团队拆活续跑时卡在假运行。
        let recoveredAt = null;
        if (r.status === 'running') {
          recoveredAt = new Date().toISOString();
          r.status = 'paused';
          r.recoveredFromRunning = {
            at: recoveredAt,
            reason: 'store_load_running_room_recovery',
          };
        }
        let recoveredTaskCount = 0;
        for (const task of r.taskList) {
          if (!task || task.status !== 'running') continue;
          recoveredAt = recoveredAt || new Date().toISOString();
          recoveredTaskCount += 1;
          task.status = 'pending';
          task.blocking = false;
          task.qualityGateFeedback = task.qualityGateFeedback || '服务重启后检测到任务停留在 running 状态;已自动恢复为可续跑任务。';
          task.recoveredFromRunning = {
            at: recoveredAt,
            reason: 'store_load_running_task_recovery',
          };
        }
        if (recoveredTaskCount > 0) {
          if (r.status !== 'paused') r.status = 'paused';
          r.recoveredFromRunning = r.recoveredFromRunning || {
            at: recoveredAt,
            reason: 'store_load_running_task_recovery',
          };
          r.recoveredTaskCount = Math.max(Number(r.recoveredTaskCount) || 0, recoveredTaskCount);
        }
        this.rooms.set(r.id, r);
      }
    } catch (e) {
      // v0.51 B-01 fix: 文件损坏时备份(原版只备 STORE_FILE,现按实际损坏的 filePath 备)
      try {
        if (existsSync(filePath)) {
          const bak = filePath + '.corrupted-' + Date.now() + '-' + process.pid + '.bak';
          copyFileSync(filePath, bak);
          console.error(`❌ ${kind} 文件 ${filePath} 损坏，已备份到 ${bak}：${e.message}`);
        } else {
          console.warn(`ChatRoomStore.load(${kind}) failed:`, e.message);
        }
      } catch (bakErr) {
        console.warn(`ChatRoomStore.load(${kind}) failed (备份也失败):`, e.message, '/', bakErr.message);
      }
    }
  }

  _doSave() {
    // v2.2 split:active 写 STORE_FILE,archived 写 ARCHIVE_FILE,各自 fingerprint 比对决定是否落盘。
    // 历史 rooms.json 含 65 房 12.8MB(22 archived 占 34% 体积),每次 active 房 turn 都重写全文件 ×3MB 写放大。
    // 拆分后:active 房 turn 只重写小主文件;archive 文件平时不动,只在 archive/unarchive/edit-archived 时改一次。
    try {
      ...
      this._writeSplit(STORE_FILE, { rooms: active }, '_lastActiveFingerprint');
      this._writeSplit(ARCHIVE_FILE, { rooms: archived }, '_lastArchiveFingerprint');
    } catch (e) {
      console.warn('ChatRoomStore.save failed:', e.message);
    }
  }

  _writeSplit(filePath, data, fingerprintKey) {
    // fingerprint 比对:只在内容真变化时才原子写盘,避免 archive 文件每次 save 都被无谓重写。
    // 用 stable-sort replacer 让 JSON.stringify 按 key 字母序输出,避免 Object.assign 后键顺序
    // 变动误判内容变化(语义未变只是序列化不稳定)。
    const json = JSON.stringify(data, stableSortReplacer, 2);
    const fp = createHash('sha256').update(json).digest('hex');
    if (this[fingerprintKey] === fp) return; // 内容没变,skip 写盘
    const tmp = filePath + '.tmp';
    // 真·SIGKILL-safe:writeFileSync 只写到 OS page cache,断电/kernel panic/强制重启时
    // 这部分尚未 fsync 到磁盘的数据丢失。显式 fsync 数据 fd + 目录 fd,确保 page cache → disk。
    // SSD 上一次 fsync ~1-5ms,squad mid-task 5 个 _persistTaskList 共 ~25ms 额外延迟,可接受。
    const fd = openSync(tmp, 'w', 0o600);
    try {
      writeFileSync(fd, json, { encoding: 'utf-8' });
      try { fsyncSync(fd); } catch { /* 某些挂载点不支持 fsync,fallback 到普通 close */ }
    } finally {
      closeSync(fd);
    }
    try { chmodSync(tmp, 0o600); } catch {}
    renameSync(tmp, filePath);
    // rename 也需要 fsync 目录 entry,防 rename 完成但目录元数据尚未落盘
    try {
      const dirFd = openSync(dirname(filePath), 'r');
      try { fsyncSync(dirFd); } catch { /* 某些 FS / 平台目录 fsync 失败属正常 */ }
      closeSync(dirFd);
    } catch { /* dir 打开失败也不阻塞 */ }
    this[fingerprintKey] = fp;
  }
  ...
  create({ name, cwd, members, mode, objective, lineage, projectContext, projectScaffold }) {
    const id = randomUUID();
    ...
    const room = {
      id,
      name: name || '未命名讨论',
      mode: ['squad', 'chat', 'debate', 'arena', 'cross_verify'].includes(mode) ? mode : 'debate',
      ...
      projectScaffold: projectScaffold || null,
