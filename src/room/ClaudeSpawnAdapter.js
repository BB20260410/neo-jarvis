...
import { applyClaudeOpus48RuntimeDefaults } from './ClaudeRuntimeDefaults.js';
...
export class ClaudeSpawnAdapter extends RoomAdapter {
  ...
  getNativeCapabilities() {
    return normalizeNativeCapabilities({
      ...
      runtime: 'Claude Code CLI (`claude --print`)',
      nativeRuntime: true,
      accountScoped: true,
      userConfigured: true,
      toolUse: true,
      skills: [
        'Claude Code 账号级能力',
        'Claude Code 本机项目上下文',
        'Claude Code 自身命令/工具使用能力',
      ],
      plugins: [
        'Claude Code 已配置的 hooks/settings',
        'Claude Code 运行时可见的扩展能力',
      ],
      tools: [
        'Claude Code 文件/命令/项目操作能力',
        'Claude Code 内置工具调用链',
      ],
      mcp: [
        'Claude CLI 原生 MCP 支持',
        '面板启用的 stdio MCP 会通过 --mcp-config 注入 Claude',
      ],
      notes: [
        '集群协同不会把 Claude 压成公共 Skill;Claude 成员按 Claude Code 自己的运行时执行。',
        'room.skills 只是额外补充提示,不替代 Claude 原生能力。',
      ],
    });
  }
  ...
  async _doChat(messages, opts = {}) {
    ...
    applyClaudeOpus48RuntimeDefaults(args, model);
