...
export function createNoeSelfEvolutionTrigger(deps = {}) {
  ...
  function noteProgress(id) { nonProgressTicks.delete(id); }
  // 记一拍无进展；达阈值则 drop goal 解锁。返回 {dropped, count}。
  function noteStuck(id) {
    ...
    if (count >= maxNonProgressTicks) {
      ...
      if (dropped) nonProgressTicks.delete(id);
  ...
  function openSelfEvolutionGoals() {
    ...
    return [...open, ...active]
      .filter((g) => g && g.source === SELF_EVOLUTION_SOURCE)
  ...
  async function tick({ goalId } = {}) {
    ...
    if (!action) {
      ...
      if (advancedThisTick) noteProgress(id); else stuck = noteStuck(id);
      return { ok: true, proposed: false, stage: loop.stage, nextAction: loop.nextAction, cycleId: cycle.cycleId,
