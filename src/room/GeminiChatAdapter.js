...
// 2026-06：Gemini 3.1 Pro Preview 是当前项目已验证的最强 Gemini 默认；调用方仍可覆盖到 Flash 做低成本路径。
...
export class GeminiChatAdapter extends RoomAdapter {
  ...
  async _doChat(messages, opts = {}) {
    ...
    try {
      ...
      // finishReason 取值如 'STOP' / 'MAX_TOKENS' / 'SAFETY'；映射成与 OpenAICompat 一致的截断字段，
      // 让 SoloChatDispatcher.isIncompleteChatResult() 能识别被 maxOutputTokens 截断的半截回复。
      // ('MAX_TOKENS' lowercase 后 = 'max_tokens'，命中现成判定，无需特判。)
      const completion = completionStatusForFinishReason(candidate?.finishReason || '');
