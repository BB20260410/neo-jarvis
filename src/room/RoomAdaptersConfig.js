// RoomAdaptersConfig — 读写 ~/.noe-panel/room-adapters.json
...
const CONFIG_DIR = join(homedir(), '.noe-panel');
...
export const DEFAULT_CONFIG = {
  minimax: {
    ...
    baseUrl: '',
    model: '',
    timeoutMs: 0,         // 0=不设置模型硬超时；>0 为用户显式覆盖
  ...
  // spawn adapter（claude/codex/ccr）默认无模型硬超时；这里仅保留用户显式覆盖
  spawn_overrides: {
    claudeTimeoutMs: 0,
    codexTimeoutMs: 0,
    ccrTimeoutMs: 0,
    minimaxSpawnTimeoutMs: 0,
  },
  customs: [
...
export function loadRoomAdaptersConfig() {
  try {
    ...
    try {
      if (existsSync(CONFIG_FILE)) {
        const bak = CONFIG_FILE + '.corrupted-' + Date.now() + '-' + process.pid + '.bak';
        copyFileSync(CONFIG_FILE, bak);
        console.error(`❌ room-adapters.json 损坏，已备份到 ${bak}：${e instanceof Error ? e.message : String(e)}`);
      }
    } catch {}
    return JSON.parse(JSON.stringify(DEFAULT_CONFIG));
  }
...
export function validateAndCleanConfig(incoming, currentConfig) {
  ...
  const cleanFlat = (target, src, fields) => {
    ...
    // timeoutMs 通用字段（0 = 不设置模型硬超时；正整数 = 用户显式覆盖。上限 7200000ms = 2h）
    if (src.timeoutMs !== undefined) {
      const n = Number(src.timeoutMs);
      if (!Number.isFinite(n) || n < 0 || n > 7200000) return 'timeoutMs 必须 0~7200000ms（0=无硬超时，最大 2 小时）';
  ...
  if (incoming.spawn_overrides && typeof incoming.spawn_overrides === 'object') {
    for (const k of ['claudeTimeoutMs', 'codexTimeoutMs', 'ccrTimeoutMs', 'minimaxSpawnTimeoutMs']) {
      if (incoming.spawn_overrides[k] !== undefined) {
        ...
        clean.spawn_overrides = clean.spawn_overrides || { claudeTimeoutMs: 0, codexTimeoutMs: 0, ccrTimeoutMs: 0, minimaxSpawnTimeoutMs: 0 };
