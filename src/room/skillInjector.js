...
import { formatNativeCapabilitiesForPrompt } from './RoomAdapter.js';

export function buildRoomAgentContext(room, options = {}) {
  if (!options?.member) return null;
  const disablePanelSkills = options?.disableSharedRoomSkills === true || options?.disablePanelSkillInjection === true;
  const roomForContext = disablePanelSkills ? { ...(room || {}), skills: [] } : room;
  return buildAgentRuntimeContext({
    room: roomForContext,
    member: options.member,
    objective: options.objective,
    codeContext: options.codeContext,
    skillStore: disablePanelSkills ? { get: () => null } : skillStore,
...
export function injectSkillsToMessages(messages, room, options = {}) {
  ...
  const nativeCapabilityPrompt = formatNativeCapabilitiesForPrompt(
    options?.nativeCapabilities || (options?.adapter && typeof options.adapter.getNativeCapabilities === 'function' ? options.adapter.getNativeCapabilities() : undefined),
  );
  if (nativeCapabilityPrompt) {
    out = appendSystemContext(out, nativeCapabilityPrompt);
  }
...
export function appendSystemContext(messages, ctx) {
...
export function getActiveSkillNames(room, options = {}, precomputedAgentContext = null) {
  if (options?.disableSharedRoomSkills === true || options?.disablePanelSkillInjection === true) return [];
