  ...
  "tasks": [
    {
      ...
      "outOfScope": ["...", "..."],         // **新增** 明确不在本 task 范围的事(防 Dev 顺手做下个 task)；2-4 项
...
- 标题: ${task.title}
- 描述（含验收标准）: ${task.desc}
${Array.isArray(task.outOfScope) && task.outOfScope.length > 0 ? `- 🚫 **不在本 task 范围**(交给后续 task 做,你写一行注释说"留给 TX"即可,**不要顺手做**):
${task.outOfScope.map((s, i) => `  ${i + 1}. ${s}`).join('\n')}` : ''}
...
` : ''}
## 📤 OUTPUT FORMAT
中文 markdown，必须含 ${isRedo ? '4' : '3'} 节（如本步未写文件,「文件落地验证」一行说明即可）：
...
如果你这一步**修改/创建了文件**，必须在这里给出**当前磁盘上实际的完整内容**——用 \`cat <文件>\` 或 \`wc -l\` + 文件读取真实读出，**禁止从记忆里复述上面的代码块**。
...
⚠️ QA 会比对你"实现/方案"段的代码 vs 这一段的真实文件，**严重不符直接 reject**。如果这一步没写文件，写一句"本步未写文件"即可。
...
- 标题: ${task.title}
- 描述（含验收标准）: ${task.desc}
${Array.isArray(task.outOfScope) && task.outOfScope.length > 0 ? `- 🚫 **不在本 task 范围**(如 Dev 顺手做了,在 issues 里指出"越界",不应阻止 pass 但要标记):
${task.outOfScope.map((s, i) => `  ${i + 1}. ${s}`).join('\n')}` : ''}
...
- "MiniMax fetch failed → 检查 \`~/.noe-panel/room-adapters.json\` 的 MiniMax baseUrl + apiKey 是否过期"
...
export class CollaborationDispatcher {
  constructor({ store, adapters, broadcast, metrics }) {
    ...
    // 可选 squad 结束钩子（server.js 注入）：done/paused/error 时回调,把 task.attempts/reviews/
    // finalConsensus 入证据知识库。解耦 room→knowledge,失败由调用方 try/catch 吞掉,不阻断 squad。
    // 签名:(roomId, room, phase) => void;phase = 'done' | 'paused' | 'error'
    this._squadFinishHook = null;
  }

  // mid-task 持久化:把当前 in-memory taskList 显式 store.update → save 到 file。
  // 修架构 bug:之前 _runOneTaskUntilTerminal 内部 push attempts/reviews / 改 status 都只在 in-memory,
  // 仅 batch 完成时(line 550)显式 store.update。SIGKILL/OOM/panic 路径(gracefulShutdown 不跑)→
  // file 滞后到上次 batch 完成,中间 attempts/reviews/status 全丢,resume 无法续跑半成品。
  // 修法:每次 attempts.push / reviews.push / task.status 改变后调一次,~50KB JSON 写盘 <10ms,
  // 一个 batch 多 4-6 次 save 可接受。
  _persistTaskList(roomId) {
    try {
      const room = this.store.get(roomId);
      if (!room) return;
      this.store.update(roomId, { taskList: room.taskList });
      // 关键!ChatRoomStore.save 是 250ms debounced,SIGKILL 在 debounce 窗口里仍丢失。
      // squad 中间状态宁可多写盘也要确保 SIGKILL-safe,显式 flush 跳过 debounce。
      if (typeof this.store.flush === 'function') this.store.flush();
    } catch (e) {
      // 持久化失败不阻断 squad 主流程(主流程仍跑 in-memory),仅警告
      console.warn('[squad-persist] taskList save failed:', e.message);
    }
  }

  // 注入 squad 结束钩子;传 null 清除。
  setSquadFinishHook(fn) {
    this._squadFinishHook = typeof fn === 'function' ? fn : null;
  }

  _fireSquadFinishHook(roomId, phase) {
    if (!this._squadFinishHook) return;
    try {
      const room = this.store.get(roomId);
      if (room) this._squadFinishHook(roomId, room, phase);
    } catch (e) { /* 索引失败不阻断 squad 主流程 */ void e; }
  }
  ...
  async start(roomId, topic, options = {}) {
    ...
    try {
      ...
      this._fireSquadFinishHook(roomId, 'done');
    } catch (e) {
      ...
      this._fireSquadFinishHook(roomId, aborted ? 'paused' : 'error');
      throw e;
    } finally {
      this.activeAborts.delete(roomId);
    }
  ...
  async _runOneTaskUntilTerminal(roomId, task, topic, members, abortSignal) {
    ...
    if (!dev || !qa) {
      ...
      this._persistTaskList(roomId); // SIGKILL-safe:配置问题 escalated 立即落盘
    ...
    while (task.iterations < task.maxIterations) {
      ...
      try {
        ...
        this._persistTaskList(roomId); // SIGKILL-safe:Dev 失败 attempt + escalated 状态立即落盘
      ...
      this._persistTaskList(roomId); // SIGKILL-safe:Dev 成功 attempt + in_review 状态立即落盘
      ...
      if (review.verdict === 'pass') {
        task.status = 'done';
        this._persistTaskList(roomId); // SIGKILL-safe:QA pass review + done 状态立即落盘
      ...
      // reject → review 已 push,持久化后进入下一次迭代
      this._persistTaskList(roomId); // SIGKILL-safe:QA reject review 立即落盘,即使下次 iter 没跑也保留打回理由
      this.broadcast(roomId, { type: 'task_qa_done', taskId: task.id, iteration: task.iterations, by: qa.displayName, review });
    }
    // 超过 maxIterations
    task.status = 'escalated';
    task.escalateReason = `QA 反复打回，达到最大迭代次数 ${task.maxIterations}`;
    this._persistTaskList(roomId); // SIGKILL-safe:max iter escalated 立即落盘
