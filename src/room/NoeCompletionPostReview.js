...
// P3 送审证据自检（2026-07-02）：pack.tests 此前恒空——m3 头号 reject 理由就是 "tests array is empty"
//   （26 条 reject blocker 几乎全是证据装配缺陷而非代码问题）。从 runtime 报告提数字装配真实测试证据；
//   读不到报告则至少带 reportRef（fail-open，绝不编数字）。
export function buildTestsEvidence({ rootAbs = process.cwd(), runtimeReportRef = '' } = {}) {
  const ref = clean(runtimeReportRef);
  if (!ref) return [];
  const entries = [`runtime-verify report: ${ref}`];
  try {
    const data = JSON.parse(readFileSync(resolve(rootAbs, ref), 'utf8'));
    const total = Number(data && (data.numTotalTests ?? data.totalTests));
    const failed = Number(data && (data.numFailedTests ?? data.failedTests));
    if (Number.isFinite(total)) {
      entries.unshift(`npm test (vitest): ${total} tests total, ${Number.isFinite(failed) ? failed : '?'} failed, ok=${data.ok === true}`);
    }
  } catch { /* fail-open：报告读不出就只带 ref，不编数字 */ }
  return entries;
}
...
// P3 反橡皮章（2026-07-02）：旧提示词明写「测试覆盖轻也放行、非重大风险别拒」——生产实测两本地 reviewer
//   601/601 全 approve（复核形同虚设）。改为怀疑者姿态：按核对清单验证证据；证据不足禁 approve（转 abstain/
//   request_changes 并点名缺口）；reject 仍留给真正确性/安全风险（配合 gate 的 reject 一票阻断 + 失败学习）。
const REVIEW_SYSTEM = [
  ...
  'Act as a skeptic, not a rubber stamp — your value is catching what green tests miss.',
  'Before approving, verify against the evidence pack: (1) touched files/diff refs match the stated goal; (2) nothing outside the goal scope was touched; (3) no caller/contract break that tests may miss; (4) no security/privacy/resource risk.',
  'If the evidence pack is too incomplete to verify these (missing diff refs, empty tests, missing runtime report), do NOT approve on faith: return "abstain" and name the missing item in evidence_gaps, or "request_changes" with the exact gap as a blocker.',
  'Use "reject" only for a genuine correctness or safety risk. "approve_with_changes" = approvable with named nits.',
  'Output ONLY a JSON verdict object — no prose, no markdown fences:',
  '{"decision":"approve"|"approve_with_changes"|"request_changes"|"reject"|"abstain","confidence":0..1,"blockers":[],"verification_required":[],"evidence_gaps":[],"consensus_vote":"yes"|"no"|"abstain"}',
].join(' ');
  ...
  return async function runPostReview({
    ...
    try {
      ...
      const pack = buildPack({
        ...
        changedFiles,
        tests: buildTestsEvidence({ rootAbs, runtimeReportRef }),
