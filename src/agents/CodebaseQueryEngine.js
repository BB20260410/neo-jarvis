...
/**
 * Tokenizes a codebase query string into a list of relevant search tokens.
 * Applies aliases for budget, diagnostics, delegation, UI, routes, and tests.
 *
 * @param {string} [text=''] - The raw query text to tokenize.
...
/**
 * Classifies a codebase query against predefined intent categories.
 *
 * @param {string} [text=''] - The raw query text.
 * @param {string[]} [tokens] - Pre-computed tokens; defaults to result of tokenizeCodebaseQuery.
 * @returns {Record<string, boolean>} An object mapping category names (e.g., 'budget', 'diagnosticsActivity') to boolean flags.
 */
...
/**
 * Calculates a relevance score for a specific file path based on query tokens and text content.
 * Uses heuristic rules for specific intents (budget, diagnostics, delegation, UI, routes) and general source/UI/test indicators.
 *
 * @param {string} [path=''] - The file path to score.
 * @param {string[]} [tokens=[]] - The list of query tokens.
 * @param {string} [text=''] - The file content or context text for additional scoring.
 * @returns {{ score: number, reasons: string[] }} An object containing the total score and a list of reasons contributing to it.
 */
...
function toClampedNumber(value, min, fallback) {
  const num = Number(value);
  return Math.max(min, Number.isFinite(num) ? num : fallback);
}

function toSlicedArray(value, max) {
...
function pushResult(results, item) {
  ...
  results.push({
    path: item.path,
    line: toClampedNumber(item.line, 1, 1),
    score: toClampedNumber(item.score, 0, 0),
    reason: uniq(item.reason || []).slice(0, 9),
    anchor: item.anchor || null,
    parser: item.parser || 'unknown',
    kind: item.kind || 'file',
    text: safeString(item.text, 260),
    symbols: toSlicedArray(item.symbols, 8),
    routes: toSlicedArray(item.routes, 8),
    bm25Rank: toFiniteNumber(item.bm25Rank),
    semanticScore: toFiniteNumber(item.semanticScore),
  });
}

/**
 * Scores and ranks codebase evidence (files, snippets) against a query.
 * Integrates full-text search (FTS) and vector search results with heuristic path scoring.
 *
 * @param {Object} map - The evidence map containing file data and metadata.
 * @param {string} [query=''] - The user's search query.
 * @param {Object} [options] - Configuration options.
 * @param {number} [options.maxResults=20] - Maximum number of results to return.
 * @param {Array} [options.ftsResults=[]] - Pre-computed full-text search results.
 * @param {Array} [options.vectorResults=[]] - Pre-computed vector search results.
 * @returns {Array} A sorted array of scored result objects.
 */
