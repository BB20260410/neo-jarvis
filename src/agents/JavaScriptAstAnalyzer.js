...
const TYPE_REFERENCE_SKIP_KEYS = new Set(['type', 'start', 'end', 'loc', 'range', 'raw', 'extra', 'comments', 'tokens', 'errors']);

function pushTypeName(out, seen, name) {
...
const TYPE_REFERENCE_HANDLERS = {
  Identifier: (node, out, seen) => pushTypeName(out, seen, typeName(node)),
  JSXIdentifier: (node, out, seen) => pushTypeName(out, seen, typeName(node)),
  TSQualifiedName: (node, out, seen) => pushTypeName(out, seen, typeName(node)),
  TSTypeReference: (node, out, seen) => {
    pushTypeName(out, seen, typeName(node.typeName));
    typeReferenceNames(node.typeParameters || node.typeArguments, out, seen);
  },
  TSExpressionWithTypeArguments: (node, out, seen) => {
    pushTypeName(out, seen, typeName(node.expression));
    typeReferenceNames(node.typeParameters || node.typeArguments, out, seen);
  },
  TSTypeQuery: (node, out, seen) => pushTypeName(out, seen, typeName(node.exprName)),
};

function typeReferenceNames(node, out = [], seen = new Set()) {
  if (!node || typeof node.type !== 'string') return out;
  const handler = TYPE_REFERENCE_HANDLERS[node.type];
  if (handler) {
    handler(node, out, seen);
    return out;
  }
  for (const [childKey, value] of Object.entries(node)) {
    if (TYPE_REFERENCE_SKIP_KEYS.has(childKey)) continue;
...
const AST_WALK_SKIP_KEYS = new Set(['type', 'start', 'end', 'loc', 'range', 'raw', 'extra', 'comments', 'tokens', 'errors']);

function isAstNode(value) {
  return !!value && typeof value.type === 'string';
}

function walkChild(value, visitor, node, childKey, ancestors) {
  if (Array.isArray(value)) {
    for (const child of value) {
      if (isAstNode(child)) walk(child, visitor, node, childKey, ancestors);
    }
    return;
  }
  if (isAstNode(value)) walk(value, visitor, node, childKey, ancestors);
}
...
function walk(node, visitor, parent = null, key = '', ancestors = []) {
  ...
  for (const [childKey, value] of Object.entries(node)) {
    if (AST_WALK_SKIP_KEYS.has(childKey)) continue;
    walkChild(value, visitor, node, childKey, nextAncestors);
  }
}
...
/**
 * Analyzes a JavaScript/TypeScript source file's AST to extract symbols, imports, exports, anchors, and references.
 *
 * @param {Object} options - The analysis options.
 * @param {string} [options.path=''] - The file path of the source code.
 * @param {string} [options.text=''] - The source code text to analyze.
 * @returns {Object} An object containing `ok`, `parser`, `sourceType`, `path`, `diagnostics`, `symbols`, `imports`, `exports`, `anchors`, `snippets`, and `references`.
 */
