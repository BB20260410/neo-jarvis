...
} from '../runtime/NoeCompletionTruthGate.js';
...
function normalizeArchiveArtifact(raw, { archive = {}, runId = '', source = 'evidenceArtifacts' } = {}) {
  ...
  const relPath = validateArtifactPath(raw);
  if (!relPath) return null;
  const resolvedRunId = resolveArtifactRunId(raw, archive, runId);
  const normalized = {
    runId: resolvedRunId,
    ...
    sessionId: resolveArtifactSessionId(raw, archive),
    gateId: resolveArtifactGateId(raw),
    reportId: str(raw.reportId, 160),
    evidenceChainId: str(raw.evidenceChainId, 160),
    latestRunId: str(raw.latestRunId, 160),
    verified: resolveArtifactVerified(raw),
    createdAt: str(archive.createdAt, 80),
    source,
    downloadable: resolveArtifactDownloadable(relPath),
  };
  normalized.id = archiveArtifactId(resolvedRunId, normalized.archiveId, normalized);
  return normalized;
}

// Helper to reduce cyclomatic complexity by extracting validation logic
function validateArtifactPath(raw) {
  const relPath = normalizeArtifactRelPath(raw.path);
  if (!relPath) return null;
  return relPath;
}

function resolveArtifactRunId(raw, archive, runId) {
  return str(raw.runId || archive.runId || runId, 160);
}

function resolveArtifactSessionId(raw, archive) {
  return str(raw.sessionId || archive.source?.sessionId, 160);
}

function resolveArtifactGateId(raw) {
  return str(raw.gateId || raw.approvalResumeGateId, 160);
}

function resolveArtifactVerified(raw) {
  return raw.verified === undefined ? null : Boolean(raw.verified);
}

function resolveArtifactDownloadable(relPath) {
  return Boolean(artifactPathDownloadRoot(relPath));
}
...
function safeAtLeastOneInt(value) {
  return Math.max(1, Number(value) || 1);
}

function safeOptionalFinite(value) {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : null;
}

function buildCitationLabel(rawLabel, path, line, fallbackId) {
  const label = str(rawLabel, 340);
  if (label) return label;
  return path ? `${path}:${line}` : fallbackId;
}

function normalizeCitationEntry(item, index) {
  const id = str(item.id, 20) || `C${index + 1}`;
  const path = str(item.path, 300);
  const line = safeAtLeastOneInt(item.line);
  const label = buildCitationLabel(item.label, path, line, id);
  return {
    id,
    path,
    line,
    ...
    semanticScore: safeOptionalFinite(item.semanticScore),
...
function normalizeCodebaseQuestionCoverage(coverage, citations) {
  const safe = coverage && typeof coverage === 'object' ? coverage : {};
  const uniquePaths = new Set(citations.map((item) => item.path).filter(Boolean));
  return {
    resultCount: Math.max(0, Number(safe.resultCount) || 0),
    citedResultCount: Math.max(0, Number(safe.citedResultCount) || citations.length),
    uniqueFileCount: Math.max(0, Number(safe.uniqueFileCount) || uniquePaths.size),
    evidenceItemCount: Math.max(0, Number(safe.evidenceItemCount) || 0),
    graphReferenceCount: Math.max(0, Number(safe.graphReferenceCount) || 0),
    routeUsageCount: Math.max(0, Number(safe.routeUsageCount) || 0),
  };
}

function resolveCodebaseQuestionCandidate(input) {
  return input?.codebaseQuestionAnswer || input?.questionAnswer || input;
}

function normalizeCodebaseQuestionCitations(rawCitations) {
...
function buildCodebaseQuestionAnswerBody(candidate, citations) {
  return {
    ...
    question: str(candidate.question, 500),
    confidence: str(candidate.confidence, 40) || 'unknown',
    answer: str(candidate.answer, 1200),
    answerLines: normalizeArray(candidate.answerLines || []).slice(0, 6),
    citations,
    coverage: normalizeCodebaseQuestionCoverage(candidate.coverage, citations),
...
function normalizeCodebaseQuestionAnswer(input = {}) {
  const candidate = resolveCodebaseQuestionCandidate(input);
  if (!candidate || typeof candidate !== 'object') return null;
  const citations = normalizeCodebaseQuestionCitations(candidate.citations);
  const body = buildCodebaseQuestionAnswerBody(candidate, citations);
  if (!body.question && !body.answer && citations.length === 0) return null;
  return body;
}
...
function buildStagedDiffFileEntry(item, index) {
  return {
...
function buildStagedDiffSummary(rawSummary, files, rawLargest) {
  return {
...
function normalizeStagedDiffReview(input = {}) {
  ...
  const files = Array.isArray(candidate.files)
    ? candidate.files.slice(0, 8).map(buildStagedDiffFileEntry).filter((item) => item.path)
    : [];
  ...
  return {
    ...
    summary: buildStagedDiffSummary(rawSummary, files, rawLargest),
    files,
  };
}

function normalizeApprovalResumeGateAuditCounts(counts) {
  const source = counts && typeof counts === 'object' ? counts : {};
  return {
    fileChanges: Math.max(0, Number(source.fileChanges) || 0),
    commands: Math.max(0, Number(source.commands) || 0),
    workEvidenceCommands: Math.max(0, Number(source.workEvidenceCommands) || 0),
    evidenceArtifacts: Math.max(0, Number(source.evidenceArtifacts) || 0),
    risks: Math.max(0, Number(source.risks) || 0),
  };
}

function normalizeApprovalResumeGateAuditFiles(files) {
  if (!Array.isArray(files)) return [];
  return files.slice(0, 8).map((item) => ({
    operation: str(item.operation, 40),
    path: str(item.path, 2000),
    beforeSha256: str(item.beforeSha256, 128),
    contentSha256: str(item.contentSha256, 128),
...
function normalizeApprovalResumeGateAuditCommands(value) {
  if (!Array.isArray(value)) return [];
...
function buildApprovalResumeGateAuditFields(candidate) {
  return {
    id: str(candidate.id || candidate.gateId, 160),
    sha256: str(candidate.sha256 || candidate.reviewSha256, 128),
    ...
    counts: normalizeApprovalResumeGateAuditCounts(candidate.counts),
    files: normalizeApprovalResumeGateAuditFiles(candidate.files),
    commands: normalizeApprovalResumeGateAuditCommands(candidate.commands),
    workEvidenceCommands: normalizeApprovalResumeGateAuditCommands(candidate.workEvidenceCommands),
...
function normalizeApprovalResumeGateAudit(input = {}) {
  const candidate = input && typeof input === 'object' ? input : {};
  const fields = buildApprovalResumeGateAuditFields(candidate);
  if (!fields.id && !fields.sha256) return null;
  return fields;
}
...
function rowToRun(row) {
  ...
  const run = {
    ...rowToRunCore(row),
    ...rowToRunReferences(row),
    ...rowToRunPayloads(row),
    ...rowToRunLifecycle(row),
  };
  return withRunLineageSummary(run);
}

function rowToRunCore(row) {
  return {
    id: row.id,
    status: row.status,
  };
...
function rowToRunReferences(row) {
  const columns = [
    ['roomId', 'room_id'],
    ['sessionId', 'session_id'],
    ['taskId', 'task_id'],
    ['agentProfileId', 'agent_profile_id'],
    ['agentProfileTitle', 'agent_profile_title'],
    ...
    ['budgetIncidentId', 'budget_incident_id'],
    ['delegationId', 'delegation_id'],
  ];
  const references = {};
  for (const [key, column] of columns) {
    references[key] = row[column] || null;
  }
  return references;
}

function rowToRunPayloads(row) {
  return {
...
function rowToRunLifecycle(row) {
  return {
...
function lineageApprovalBlocker(run, deferReason, lineage) {
  if (run.status !== 'deferred' || !/approval/i.test(deferReason || '')) return null;
  const pending = (lineage.approvals || []).find((item) => item.status === 'pending') || lineage.approvals?.[0];
...
function lineageBudgetBlocker(run, deferReason, lineage) {
  const isBudgetDeferred = run.status === 'deferred' && /budget/i.test(deferReason || '');
  const openBudget = (lineage.budgetIncidents || []).find((item) => item.status === 'open');
  if (!isBudgetDeferred && !openBudget) return null;
  const firstIncident = lineage.budgetIncidents?.[0];
  return {
    kind: 'budget',
    id: openBudget?.id || firstIncident?.id || run.budgetIncidentId || run.details?.budgetIncidentId || null,
...
function lineageFailureBlocker(run) {
...
function lineageBlockers(run = {}, lineage = {}) {
  ...
  return [
    lineageApprovalBlocker(run, deferReason, lineage),
    lineageBudgetBlocker(run, deferReason, lineage),
    lineageFailureBlocker(run),
  ].filter(Boolean);
}
...
function resolveReplayStatus(input = {}) {
  return str(input.status || 'recorded', 80) || 'recorded';
}

function resolveReplayRecordedBy(input = {}) {
  return str(input.requestedBy || input.actorType || 'system', 120) || 'system';
}

function resolveReplaySummary(input = {}, status) {
  return str(input.summary || input.outputSummary || `Replay result ${status}`, 2000) || `Replay result ${status}`;
}

function resolveReplayPlanId(input = {}, messages = []) {
  return str(input.replayPlanId || latestReplayPlanId(messages), 160);
}

function buildReplayResult(timeline = {}, input = {}) {
  const run = timeline.run || {};
  const messages = timeline.messages || [];
  const status = resolveReplayStatus(input);
  return {
    id: `replay-result-${randomUUID().slice(0, 12)}`,
    runId: run.id,
    replayPlanId: resolveReplayPlanId(input, messages),
    status,
    summary: resolveReplaySummary(input, status),
    recordedAt: new Date().toISOString(),
    recordedBy: resolveReplayRecordedBy(input),
    safeToAutoExecute: false,
...
const GATE_AUDIT_PARTITION_MISSING_FIELD_HANDLERS = {
  artifact: (source) => (source.partitionDigests?.artifact ? [] : ['artifact']),
  file: (_source, diff) => (diff ? [] : ['stagedDiffReview']),
  command: (_source, diff, audit) => {
    const missing = [];
  ...
  coverage: (_source, diff) => (diff ? [] : ['stagedDiffReview.coverage']),
  risk: (_source, diff) => (diff ? [] : ['stagedDiffReview.risk']),
};

function gateAuditPartitionMissingFields(source = {}, partition) {
  const audit = source.audit || {};
  const diff = audit.stagedDiffReview || null;
  const handler = GATE_AUDIT_PARTITION_MISSING_FIELD_HANDLERS[partition];
  return handler ? handler(source, diff, audit) : [];
}
...
function buildIdeaRunDraft(input = {}) {
  const { classification, inputDetails, idea, affectedFiles, matches, profile, skills, dispatchTags, governance, codebaseQuestionAnswer } = extractIdeaInputs(input);

function extractIdeaInputs(input) {
  ...
  return { classification, inputDetails, idea, affectedFiles, matches, profile, skills, dispatchTags, governance, codebaseQuestionAnswer };
}
  const plan = {
...
function modelPatchTargetForAffectedFiles(files = []) {
  ...
  return files.find((file) => /^(src|public|tests|docs)\//.test(file) && allowedExt.test(file) && !/\.test\.[cm]?js$/.test(file))
    ...
    || 'docs/noe-agent-skill-registry.md';
}
...
export class AgentRunStore {
  ...
  /**
   * Exact COUNT for a status (health drain must not use list limit=500 which caps at 500).
   * @param {string} [status]
   * @returns {number}
   */
  countByStatus(status) {
    const st = status ? normalizeStatus(status) : '';
    if (st) {
      const row = this.db().prepare('SELECT COUNT(*) AS c FROM agent_runs WHERE status = ?').get(st);
      return Number(row?.c) || 0;
    }
    const row = this.db().prepare('SELECT COUNT(*) AS c FROM agent_runs').get();
    return Number(row?.c) || 0;
  }
  ...
  getSessionSnapshot(sessionId, options = {}) {
    ...
    // 共享缓存：让全部 run 的 getTimeline 只拉一次“全量近期审计”，
    // 把原本 N 次相同的 audit.list({ order:'ASC', limit:1000 }) 合并为 1 次，结果等价。
    const timelineCache = {};
    const timelines = rows.map((row) => this.getTimeline(row.id, timelineCache)).filter(Boolean);
  ...
  transition(id, status, details = {}) {
    ...
    const now = nowMs();
    let nextDetails = { ...(run.details || {}), ...(details || {}) };

    // S2: optional completion truth gate (env-gated default off for safe migration).
    // When enabled, never write succeeded if exit/verify/evidence fail-closed checks deny.
    const truthGateOn = String(process.env.NOE_COMPLETION_TRUTH_GATE || '0') === '1';
    if (truthGateOn && normalizeTerminalStatus(normalizedStatus) === 'completed') {
      const decision = evaluateCompletionTruth({
        requestedStatus: 'completed',
        exitCode: details.exitCode !== undefined ? details.exitCode : nextDetails.exitCode,
        verified: details.verified !== undefined ? details.verified : nextDetails.verified,
        hasValidArtifacts: details.hasValidArtifacts !== undefined ? details.hasValidArtifacts : nextDetails.hasValidArtifacts,
        hasEvidence: details.hasEvidence !== undefined ? details.hasEvidence : nextDetails.hasEvidence,
        validatorsPass: details.validatorsPass !== undefined ? details.validatorsPass : nextDetails.validatorsPass,
        sourceDigestMatch: details.sourceDigestMatch !== undefined ? details.sourceDigestMatch : nextDetails.sourceDigestMatch,
        approvalsSettled: details.approvalsSettled !== undefined ? details.approvalsSettled : nextDetails.approvalsSettled,
        highRiskActsSettled: details.highRiskActsSettled !== undefined ? details.highRiskActsSettled : nextDetails.highRiskActsSettled,
        error: details.error || details.message || null,
        dryRun: details.dryRun === true || nextDetails.dryRun === true,
      }, { strict: true });
      if (!decision.allowed) {
        ...
        nextDetails = {
          ...
          completionTruthRequested: status,
          completionTruthFinal: decision.finalStatus,
        };
        if (!details.error && !details.message) {
          details = {
            ...details,
            error: `completion_truth_denied:${(decision.blockers || []).join(',')}`,
          };
        }
      } else {
        nextDetails = {
          ...
          completionTruthVersion: decision.version,
        };
      }
    }
    ...
    const error = normalizedStatus === 'failed' ? str(details.error || details.message || nextDetails.error, 4000) : null;
  ...
  /**
   * Reap zombie agent_runs left in status=running after crash/restart (no finished_at).
   * Bulk SQL (not per-row transition) — panel.db can hold 100k+ stale running rows; health
   * lists limit 500 so unreaped zombies pin runningTaskCount at 500 forever.
   *
   * Default: only touch rows with updated_at older than 30m (spare truly active work).
   * @param {{ now?: number, olderThanMs?: number, limit?: number, reason?: string }} [opts]
   * @returns {{ recovered: number, cutoff: number, olderThanMs: number }}
   */
  recoverStaleRunningRuns({
    ...
    const ts = Number(now) || nowMs();
    const age = Math.max(60_000, Number(olderThanMs) || 30 * 60 * 1000); // min 1m
    const cutoff = ts - age;
    const lim = Math.max(1, Math.min(200_000, Number(limit) || 50_000));
    const errText = str(reason, 400) || 'stale_running_recovered';
    // Count first for observability (cheap index scan on status).
    const before = Number(
      this.db().prepare(
        `SELECT COUNT(*) AS c FROM agent_runs
         WHERE status = 'running' AND finished_at IS NULL AND updated_at <= ?`,
      ).get(cutoff)?.c,
    ) || 0;
    if (before === 0) {
      return { recovered: 0, cutoff, olderThanMs: age, remainingStale: 0 };
    }
    // Batch update: mark failed + stamp finished_at; leave recent running alone.
      ...
      SET status = 'failed',
          ...
          details = json_set(
            CASE
              ...
              ELSE details
            END,
            '$.staleRecovered', json('true'),
            '$.staleRecoveredAt', ?,
            '$.staleRecoveredReason', ?
          )
      WHERE id IN (
    ...
    `).run(ts, ts, errText, ts, errText, cutoff, lim);
    const recovered = Number(result?.changes) || 0;
    const remainingStale = Number(
      this.db().prepare(
        `SELECT COUNT(*) AS c FROM agent_runs
         WHERE status = 'running' AND finished_at IS NULL AND updated_at <= ?`,
      ).get(cutoff)?.c,
    ...
    if (recovered > 0) {
      try {
        this.logger?.warn?.(
          `[agent-runs] recoverStaleRunningRuns recovered=${recovered} remainingStale=${remainingStale} olderThanMs=${age}`,
        );
      } catch { /* ignore */ }
      try {
        this.audit?.recordSafe?.({
          action: 'agent.run.stale_recovered_batch',
          actorType: 'system',
          entityType: 'agent_runs',
          status: 'failed',
          severity: 'warn',
          details: { recovered, remainingStale, olderThanMs: age, cutoff, reason: errText },
        });
      } catch { /* ignore */ }
    }
    return { recovered, cutoff, olderThanMs: age, remainingStale };
  }
  ...
  // cache：可选共享缓存对象（如 getSessionSnapshot 批量调用时传入）。
  // “recentEvents”（无 run 维度的全量近期审计）每个 run 查出来都一样，
  // 故在一次批量里只拉一次复用——结果与逐 run 各拉一次完全等价，仅消除 N 次重复全表扫描。
  relatedActivityEvents(run, cache = null) {
    ...
    let recentEvents;
    if (cache && Array.isArray(cache.recentEvents)) {
      recentEvents = cache.recentEvents;
    } else {
      recentEvents = this.audit.list({ order: 'ASC', limit: 1000 });
      if (cache) cache.recentEvents = recentEvents;
    }
  ...
  // cache：可选共享缓存，仅透传给 relatedActivityEvents 用于批量去重；
  // 不传时行为与原先逐个调用完全一致（外部所有单 run 调用方走此分支）。
  getTimeline(id, cache = null) {
    const run = this.get(id);
    if (!run) return null;
    ...
    const activityEvents = this.relatedActivityEvents(run, cache);
