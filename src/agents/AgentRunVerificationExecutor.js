...
/**
 * Builds a safe environment object for spawning verification child processes.
 *
 * This function filters the provided environment (defaulting to `process.env`)
 * to include only whitelisted variables, preventing leakage of sensitive secrets
 * (e.g., API keys) to child processes during self-evolution test runs.
 *
 * @param {Record<string, string>} [env=process.env] - The source environment variables.
 * @returns {Record<string, string>} A sanitized environment object safe for child process spawning.
 */
export function buildVerificationSpawnEnv(env = process.env) {
  return buildNoeSafeChildProcessEnv(env);
}
