// Noe v2.0 — Lemon Squeezy REST API 集成 endpoint
// 让 panel UI 能查询 LS 状态 + 创建 checkout / webhook

import { requireOwnerToken } from '../auth/owner-token.js';
import { normalizeWebhookSecretInput } from './payment-webhooks.js';
...
export function registerLemonSqueezyRoutes(app) {
  ...
  app.post('/api/lemonsqueezy/webhook-auto-register', requireOwnerToken, async (req, res) => {
    try {
      ...
      if (!storeId || !url || secret === undefined) {
      ...
      if (!normalizedSecret.ok) return res.status(400).json({ ok: false, error: normalizedSecret.error });
      const m = await import('../../integrations/LemonSqueezyClient.js');
      const r = await m.createWebhook({ storeId, url, secret: normalizedSecret.value });
      // 自动把 secret 存到 panel webhook-secrets.json
      const fs = await import('node:fs');
      const path = await import('node:path');
      const os = await import('node:os');
      const secretsPath = path.join(os.homedir(), '.noe-panel', 'webhook-secrets.json');
      const secrets = fs.existsSync(secretsPath) ? JSON.parse(fs.readFileSync(secretsPath, 'utf8')) : {};
      secrets.lemon = normalizedSecret.value;
