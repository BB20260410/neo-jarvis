...
export function registerMcpRoutes(app, deps) {
  ...
  // 调用 MCP server 的工具（callTool）。受信任本地 server（unified-kb 等）经 classify 白名单放行免审批；
  // 其它 server 走 requirePermission 审批。危险写工具(如 fs_organize_execute)还有目标 server 自己的 confirm 二道防线。
  app.post('/api/mcp/servers/:name/call', requireOwnerToken, async (req, res) => {
    try {
      const body = req.body || {};
      const toolName = String(body.toolName || body.tool || '').slice(0, 200);
      if (!toolName) return res.status(400).json({ ok: false, error: 'toolName required' });
      if (JSON.stringify(body).length > 256 * 1024) return res.status(413).json({ ok: false, error: 'body 过大' });
      if (!requirePermission(req, res, {
        action: 'skill.plugin.execute',
        target: mcpTarget('call', req.params.name, { toolName }),
      })) return;
      const result = await mcpClientManager.callTool(req.params.name, toolName, body.args || body.arguments || {});
