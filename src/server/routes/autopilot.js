...
export function registerAutopilotRoutes(app, deps) {
  ...
  // A1 安全修复：log/runs/schedules 含任务 prompt 与执行痕迹，读端点同样必须 owner-token（防本机进程裸读）
  app.get('/api/autopilot/config', requireOwnerToken, (req, res) => {
  ...
  app.get('/api/autopilot/log', requireOwnerToken, (req, res) => {
    try {
  ...
  app.get('/api/autopilot/schedules', requireOwnerToken, (req, res) => {
    try {
  ...
  app.get('/api/autopilot/jobs', requireOwnerToken, (req, res) => {
    try {
  ...
  app.get('/api/autopilot/runs', requireOwnerToken, (req, res) => {
    try {
  ...
  app.post('/api/autopilot/dry-run', requireOwnerToken, async (req, res) => { // 强健工程：同文件唯一漏 token 的写端点
    try {
