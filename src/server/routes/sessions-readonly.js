...
// A1 安全修复：成本时序/安全历史含会话使用痕迹，读端点同样必须 owner-token（防本机进程裸读）。
...
export function registerSessionsReadonlyRoutes(app, deps) {
  ...
  app.get('/api/sessions/:id/cost-series', requireOwnerToken, (req, res) => {
    try {
  ...
  app.get('/api/sessions/:id/safety-history', requireOwnerToken, (req, res) => {
    try {
      const s = sessions.get(req.params.id);
      if (!s) return res.status(404).json({ error: 'not found' });
      res.json({
