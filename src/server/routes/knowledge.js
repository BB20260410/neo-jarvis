...
import { createNoeCrossLayerSearch } from '../../knowledge/NoeCrossLayerSearch.js';

export function registerKnowledgeRoutes(app, deps) {
  const { knowledgeStore, evidenceKnowledgeStore, agentRunStore, activityLog, llmWikiRoot, wikiSearch = searchWiki, memoryCore = null } = deps;
  ...
  app.get('/api/knowledge/llm-wiki/search', requireOwnerToken, async (req, res) => {
    try {
      const query = String(req.query?.q || req.query?.query || '');
      const topK = req.query?.topK ? Number(req.query.topK) : undefined;
      const result = await wikiSearch({ root: llmWikiRoot, query, topK });
      res.json(result);
    } catch (e) { res.status(400).json({ ok: false, error: e.message }); }
  });
  ...
  // P3-5 跨层融合查询：一次 query 并行查多层（文档 KB[需 kb 名] + 证据 FTS + 记忆[若注入]），RRF 融合排序，
  //   解「文档/证据两条平行独立端点各查各的」孤岛。单层失败 graceful 跳过，结果带 layers/crossLayer 标注。
  app.post('/api/knowledge/cross-search', requireOwnerToken, async (req, res) => {
    try {
      const query = String(req.body?.query || req.body?.q || '');
      const kb = req.body?.kb ? String(req.body.kb) : null;
      const limit = req.body?.limit ? Number(req.body.limit) : 10;
      const layers = [];
      if (kb && knowledgeStore) layers.push({ name: 'docs', search: async (q, { limit: l }) => { const h = await knowledgeStore.search({ name: kb, query: q, topK: l, hybrid: true }); return Array.isArray(h) ? h : (h?.hits || []); } });
      if (evidenceKnowledgeStore) layers.push({ name: 'evidence', search: async (q, { limit: l }) => evidenceKnowledgeStore.search(q, { limit: l }) });
      if (typeof memoryCore?.recall === 'function') layers.push({ name: 'memory', search: async (q, { limit: l }) => { const h = await memoryCore.recall({ query: q, limit: l }); return Array.isArray(h) ? h : (h?.items || h?.memories || []); } });
      const r = await createNoeCrossLayerSearch({ layers }).search(query, { limit });
      res.json(r);
    } catch (e) { res.status(400).json({ ok: false, error: e.message }); }
  });
