...
//
// 安全模型：
//   - panel 仅本地监听 127.0.0.1:51835（不对外）
...
const AUDIT_LOG = path.join(os.homedir(), '.noe-panel', 'auto-fill-audit.jsonl');
...
export function registerAutoFillRoutes(app) {
  ...
  app.post('/api/auto-fill/password', requireOwnerToken, async (req, res) => {
    ...
    // 严格域名匹配：用 URL.hostname 精确比对，避免 includes 被
    // "evil.com.attacker.com" 这类子串绕过（site 必须等于 hostname 或为其父域）
    let hostMatches = false;
    try {
      const host = new URL(url).hostname.toLowerCase();
      const target = site.toLowerCase();
      hostMatches = host === target || host.endsWith('.' + target);
    } catch { hostMatches = false; }
    if (!hostMatches) {
