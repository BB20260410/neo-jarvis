...
const HOME = path.join(os.homedir(), '.noe-panel');
const KEYS_DIR = path.join(os.homedir(), '.noe-panel-keys');
const COMMERCIAL_STATUS_TTL_MS = 5000;
let commercialStatusCache = { at: 0, value: null };
...
function getCommercialStatus() {
  const status = {
    ...
    lemonWebhook: {
      ...
      script: 'curl -X POST http://localhost:51835/api/webhooks/config -d \'{"provider":"lemon","secret":"<your_secret>"}\'',
    },
    polarWebhook: {
      done: (() => {
        const sp = path.join(HOME, 'webhook-secrets.json');
      ...
      script: 'curl -X POST http://localhost:51835/api/webhooks/config -d \'{"provider":"polar","secret":"<your_secret>"}\'',
    },
    sqliteMigrated: {
    ...
    pricingPage: {
      ...
      script: 'open http://localhost:51835/pricing.html',
    },
  };
...
function getCommercialStatusCached(now = Date.now()) {
  if (commercialStatusCache.value && now - commercialStatusCache.at < COMMERCIAL_STATUS_TTL_MS) return commercialStatusCache.value;
  const value = getCommercialStatus();
  commercialStatusCache = { at: now, value };
  return value;
}

export function resetCommercialStatusCacheForTest() {
  commercialStatusCache = { at: 0, value: null };
}

export function registerCommercialSetupRoutes(app) {
  // 审计 §3.1 P0-4：行内 requireOwnerToken，不再只靠全局中间件兜底（隐式安全合同难审计）。
  // 这两个端点会暴露 webhook-secret / license-key 等敏感文件是否存在。
  app.get('/api/commercial/status', requireOwnerToken, (req, res) => {
    try {
      res.json({ ok: true, ...getCommercialStatusCached() });
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
    }
  });

  app.get('/api/commercial/next-step', requireOwnerToken, (req, res) => {
    try {
      const s = getCommercialStatusCached();
