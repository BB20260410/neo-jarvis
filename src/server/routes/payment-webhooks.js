...
const SECRETS_PATH = path.join(HOME, '.noe-panel', 'webhook-secrets.json');
const ISSUED_LOG = path.join(HOME, '.noe-panel', 'licenses-issued.jsonl');
const PRIV_KEY_PATH = path.join(HOME, '.noe-panel-keys', 'panel-license-private-key.pem');
...
export function buildIssuedWebhookResponse({ email = '', tier = '' } = {}) {
  return { ok: true, issued: true, email, tier, licenseReturned: false };
}

export function normalizeWebhookSecretInput(secret) {
  if (typeof secret !== 'string') return { ok: false, error: 'secret must be a string' };
  const value = secret.trim();
  if (value.length < 16) return { ok: false, error: 'secret 至少 16 字符' };
...
export function registerPaymentWebhookRoutes(app) {
  ...
  app.post('/api/webhooks/config', requireOwnerToken, (req, res) => {
    try {
      ...
      const normalizedSecret = normalizeWebhookSecretInput(secret);
      if (!normalizedSecret.ok) return res.status(400).json({ error: normalizedSecret.error });
      const s = loadSecrets();
      s[provider] = normalizedSecret.value;
      saveSecrets(s);
  ...
  app.post('/api/webhooks/lemon', async (req, res) => {
    ...
    try {
      ...
      res.json(buildIssuedWebhookResponse({ email, tier }));
    } catch (e) {
