...
export function installNoeMaintenanceLoops({ memoryCore, prefetchStore, dataDir }) {
  ...
  if (process.env.NOE_DB_BACKUP !== '0') {
    // P1（2026-07-02）：keep 可由 NOE_DB_BACKUP_KEEP 配置（panel.db 已 2GB 级，7 份≈14GB 磁盘；默认仍 7 零回归）
    const keepBackups = Math.max(1, Number(process.env.NOE_DB_BACKUP_KEEP) || 7);
    const runDbBackup = () => backupPanelDb({ keep: keepBackups })
    ...
    dbBackupTimer.unref?.();
    console.log(`[noe-db-backup] 已启用（启动 90s 后 + 每 24h 快照，保留 ${keepBackups} 份）`);
  }
