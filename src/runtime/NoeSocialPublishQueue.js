...
// R2-P0（2026-07-03）：不可逆副作用防重复的关键——在真实点击「发布」之前预落 attempt 标记。
//   若点击后进程崩溃/落标失败，磁盘已有 externalSideEffectAttempted=true，重试被去重闸挡住，绝不重复对外发布。
//   幂等：已 published（externalSideEffectPerformed=true）的 draft 不回退；仅补 attempted 痕迹，不改 state 为 published
//   （真发布确认仍由 markNoeSocialDraftExternalSideEffect 负责）。
export function markNoeSocialDraftExternalSideEffectAttempt({
  dir = DEFAULT_NOE_SOCIAL_DRAFT_DIR,
  id = '',
  reason = 'external_publish_attempt_started',
  ...
  const record = {
    ...
    publish: {
      ...
      attemptedAt: clean(priorPublish.attemptedAt || attemptedAt, 100),
    },
  };
  delete record.sha256;
  const next = { ...record, sha256: sha256(stableJson(record)) };
  ...
  return {
    ...
    id: clean(id, 180),
    externalSideEffectAttempted: true,
    attemptedAt: next.publish.attemptedAt,
    path: current.path,
    ref: basename(current.path),
    sha256: next.sha256,
  };
