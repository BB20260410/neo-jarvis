...
export const BAILONGMA_FUSION_SCHEMA_VERSION = 2;

/** Fixed comparison baseline for surpass contract (S0 snapshot). */
export const BAILONGMA_FIXED_BASELINE = {
  release: 'v2.1.549',
...
/**
 * Product-surpass dimensions (install → loop → browser/voice/memory → evidence → package).
 * Structural detection only — never implies Neo product verified.
 * @param {{ inventory?: any, packageJson?: object }} [opts]
 */
export function buildProductSurpassFeatureMap({ inventory, packageJson = {} } = {}) {
  const files = fileSet(inventory);
  const deps = packageDeps(packageJson);
  return [
    {
      id: 'product_onboarding_first_task',
      dimension: 'install_onboarding',
      title: 'Install → Doctor → first real task',
      detected: hasAny(files, ['README.md', 'package.json']) && (files.has('src/index.js') || files.has('src/main.js') || files.has('electron/main.js') || files.has('src/app.js')),
      upstreamEvidence: ['package.json', 'README.md'],
      noeFusion: 'Neo must beat ≤10 min install-to-first-verified-task with Doctor + receipt, not marketing copy.',
      risks: ['guided_only_onboarding', 'missing_doctor', 'secret_in_setup'],
    },
    {
      id: 'product_default_task_loop',
      dimension: 'default_task_loop',
      title: 'Default one-shot task loop with status',
      detected: hasAny(files, ['src/runtime/loop.js', 'src/agent/loop.js', 'src/runtime/agent-loop.js', 'src/core/agent.js']),
      upstreamEvidence: ['src/runtime', 'src/agent'],
      noeFusion: 'Map into UnifiedTaskStore + AgentRuntime; fail closed on false completion; reuse Autopilot/ToolRegistry/ActPipeline.',
      risks: ['linear_single_task_kernel', 'soft_review_as_complete'],
    },
    {
      id: 'product_browser_loop',
      dimension: 'browser',
      title: 'Browser research/action product surface',
      detected: hasAny(files, ['src/browser', 'src/tools/browser.js', 'src/capabilities/browser.js']) || depPresent(deps, 'playwright') || depPresent(deps, 'puppeteer'),
      upstreamEvidence: ['browser modules or playwright/puppeteer dep'],
      noeFusion: 'Ship real browser E2E with evidence artifacts; no unknown side-effect replay.',
      risks: ['headed_only', 'no_artifact_hash'],
    },
    {
      id: 'product_voice_loop',
      dimension: 'voice',
      title: 'Voice dispatch / approve loop',
      detected: hasAny(files, ['src/voice', 'src/audio', 'src/speech']) || depPresent(deps, 'mic') || depPresent(deps, 'node-record-lpcm16'),
      upstreamEvidence: ['voice/audio modules'],
      noeFusion: 'Chinese voice path must share Task/approval truth with text front door.',
      risks: ['voice_parallel_state', 'approval_desync'],
    },
    {
      id: 'product_memory_loop',
      dimension: 'memory',
      title: 'Memory write/recall/correct/export',
      detected: hasAny(files, ['src/memory', 'src/db.js', 'src/memory.js']),
      upstreamEvidence: ['src/memory or db'],
      noeFusion: 'Standard memory set: recall≥85% precision≥90% cross-project sensitive misuse=0.',
      risks: ['policy_memory_as_self_evolution', 'secret_in_memory'],
    },
    {
      id: 'product_evidence_status',
      dimension: 'evidence_truth',
      title: 'User-visible task status and evidence',
      detected: hasAny(files, ['src/ui', 'src/frontend', 'public']) || files.has('src/hotspots.js'),
      upstreamEvidence: ['UI surfaces'],
      noeFusion: 'Receipt-first UI; expert mode same Task/Evidence truth; never complete on model claim alone.',
      risks: ['decorative_brain_ui', 'status_without_ledger'],
    },
    {
      id: 'product_packaging_update',
      dimension: 'packaging_update',
      title: 'Desktop install package and update story',
      detected: hasAny(files, ['electron-builder.yml', 'electron-builder.json', 'forge.config.js']) || depPresent(deps, 'electron') || depPresent(deps, 'electron-builder'),
      upstreamEvidence: ['electron packaging config'],
      noeFusion: 'macOS universal sign/notarize + N-1 upgrade/rollback; Windows CI; no release/main split.',
      risks: ['unsigned_identity_null', 'out_vs_out-noe_mismatch'],
    },
    {
      id: 'product_security_sandbox',
      dimension: 'permission_security',
      title: 'Tool policy / sandbox claims',
      detected: hasAny(files, ['src/capabilities/tool-policy.js', 'src/runtime/tool-protocol.js', 'src/security']),
      upstreamEvidence: ['tool-policy', 'tool-protocol'],
      noeFusion: 'Reject string shell, regex pseudo-sandbox, new Function marketplace; require Plugin Host + OS sandbox fail-closed.',
      risks: ['regex_sandbox', 'main_process_dynamic_import'],
    },
  ];
}
...
export function buildBaiLongmaFeatureMap({ inventory, packageJson = {} } = {}) {
  ...
  const product = buildProductSurpassFeatureMap({ inventory, packageJson });
  return [...socialAndTools, ...product];
}
...
export function buildBaiLongmaFusionReport({
  bailongmaRoot,
  upstreamUrl = BAILONGMA_FIXED_BASELINE.upstreamUrl,
  upstreamCommit = '',
  fixedBaseline = BAILONGMA_FIXED_BASELINE,
  noePackageJson = {},
  noeCapabilities = {},
  env = {},
  ...
  const productFeatures = features.filter((f) => f.dimension);
  ...
  return {
    ...
    generatedAt,
    fixedBaseline: {
      release: fixedBaseline.release,
      commit: fixedBaseline.commit,
      upstreamUrl: clean(fixedBaseline.upstreamUrl || upstreamUrl, 300),
    },
    source: {
      upstreamUrl: clean(upstreamUrl, 300),
      upstreamCommit: clean(upstreamCommit || fixedBaseline.commit, 80),
