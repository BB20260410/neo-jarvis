...
export class NoeTurnContextEngine {
  ...
  constructor({
    ...
    // 第三阶段·全模态一体化:每轮融合的多模态感知(视觉/语音/UI)provider——让聊天大脑跨模态推理(不只文字)。
  ...
  async supplyTurnContext({
    ...
    // 第三阶段·全模态一体化（NOE_CHAT_MULTIMODAL=1 默认 OFF 零回归）：每轮融合当下多模态感知(视觉屏幕/语音/UI 操作)注入,
    //   让聊天大脑跨模态理解「你此刻在干嘛」,而不是只看文字。keep:2(转瞬即逝,预算压力下先裁);fail-open。
    if (on('multimodal') && this.multimodalProvider && process.env.NOE_CHAT_MULTIMODAL === '1') {
      const block = await runProvider('multimodal', () => this.multimodalProvider({ transcript, projectId }));
      if (block) ctx.add('multimodal', String(block), { keep: 2 });
    }
