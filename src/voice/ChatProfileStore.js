...
import { splitStablePrefix } from '../context/NoePromptPrefix.js';
...
export class ChatProfileStore {
  ...
  resolve(id) {
    ...
    // R2-P3 prefix cache 接线（NOE_PROMPT_PREFIX=1 默认 OFF，零回归）：把 host/continuity 里的易变行
    //   （时间戳/日期/电量数字/时段）剥到末尾 <runtime> 块，让稳定前缀逐轮不变命中 provider prefix cache
    //   （省 30-60% 输入 token）。split 只作用于 BOUNDARY 之前的 body，BOUNDARY 始终留最末（安全边界不被顶掉）。
    const body = `${p.systemPrompt}${skBlock}${hostBlock}${continuityBlock}`;
    const stabilized = process.env.NOE_PROMPT_PREFIX === '1' ? splitStablePrefix(body).combined : body;
    return { ...p, systemPrompt: `${stabilized}\n\n${BOUNDARY}` };
  }
