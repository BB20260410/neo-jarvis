...
function evaluateValidator(artifact = {}, errors = []) {
  const validator = isPlainObject(artifact.validator) ? artifact.validator : {};
  if (!isPlainObject(artifact.validator)) {
    errors.push('artifact_validator_required');
    return;
  }
  if (!hasText(validator.validatorVersion)) errors.push('artifact_validator_version_required');
