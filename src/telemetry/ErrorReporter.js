...
export async function captureException(err, opts = {}) {
  try {
    ...
    // L9 修复：限流 Map 只增不删会在长跑进程中无界增长；超阈值时清掉已过限流窗口的过期指纹。
    if (_recentFingerprints.size > 500) {
      const cutoff = Date.now() - RATE_LIMIT_MS;
      for (const [k, ts] of _recentFingerprints) { if (ts < cutoff) _recentFingerprints.delete(k); }
    }
