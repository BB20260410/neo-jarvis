...
// 每个 workspace 独立目录：~/.noe-panel/workspaces/{name}/
// 默认 workspace 名 'default'，向后兼容旧 ~/.noe-panel/ 直接放数据的形态
...
export function listWorkspaces() {
  ...
  if (fs.existsSync(WORKSPACES_DIR)) {
    for (const entry of fs.readdirSync(WORKSPACES_DIR)) {
      ...
      items.push({ name: entry, builtin: false, createdAt: null, ...meta });
    }
  }
  return items;
}
...
export function getWorkspaceDir(name = null) {
  ...
  if (ws === DEFAULT_NAME) return HOME; // default workspace 直接落 ~/.noe-panel/ 兼容旧版
