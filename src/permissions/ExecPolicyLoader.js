...
export function loadExecPolicyStore(opts = {}) {
  ...
  let trustLevel;
  if (VALID_LEVELS.has(envLevel)) { trustLevel = envLevel; source = 'env'; }
  else if (VALID_LEVELS.has(config.trustLevel)) trustLevel = config.trustLevel;
  else trustLevel = VALID_LEVELS.has(opts.defaultTrust) ? opts.defaultTrust : 'developer';
