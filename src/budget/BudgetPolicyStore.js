...
export class BudgetPolicyStore {
  ...
  buildMetricPayload(summary) {
    const pickOrNull = (value) => value || null;
    const pickArray = (value) => (Array.isArray(value) ? value : []);
    return {
      roomMode: pickOrNull(summary.roomMode),
      roomName: pickOrNull(summary.roomName),
      turn: pickOrNull(summary.turn),
      model: pickOrNull(summary.model),
      success: summary.success !== false,
      errorKind: pickOrNull(summary.errorKind),
      agentRunId: pickOrNull(summary.agentRunId),
      agentProfileId: pickOrNull(summary.agentProfileId),
      agentProfileTitle: pickOrNull(summary.agentProfileTitle),
      agentDispatchTags: pickArray(summary.agentDispatchTags),
      agentSkillNames: pickArray(summary.agentSkillNames),
      agentGovernance: pickOrNull(summary.agentGovernance),
    };
  }

  buildUsageInfo(scope, metric, ts, summary, payload) {
    return {
      ts,
  ...
  collectScopeChecks(scopes, amounts, ts, payload) {
    const checks = [];
  ...
  recordMetric(summary = {}) {
    ...
    `);
    const payload = this.buildMetricPayload(summary);
    const inserted = [];
    for (const scope of scopes) {
      for (const metric of amounts) {
        const info = this.buildUsageInfo(scope, metric, ts, summary, payload);
    ...
    const checks = this.collectScopeChecks(scopes, amounts, ts, payload);
    return {
  ...
  checkScope({ scopeType, scopeId, metric = 'usd', estimateAmount = 0, ts = nowMs(), source = 'manual', details = {} } = {}) {
    ...
    for (const policy of policies) {
      const result = this._evaluatePolicy(policy, estimateAmount, ts, source, details);
  ...
  _evaluatePolicy(policy, estimateAmount, ts, source, details) {
    ...
    const hardStopReached = this.computeHardStopReached(source, observed, projected, policy.amount);
    const incidents = [];
    const warnings = [];
    const blocked = [];
    if (!hardStopReached) {
      const warnEntry = this.buildWarningEntry(policy, projected, warnLimit, ts, source, details);
      if (warnEntry) {
        incidents.push(warnEntry.incident);
        warnings.push(warnEntry);
      }
    }
    if (hardStopReached) {
      const { incident, blockedItem } = this.buildHardStopEntry(policy, projected, ts, source, details);
      if (incident) incidents.push(incident);
      if (policy.hardStopEnabled) blocked.push(blockedItem);
      else warnings.push(blockedItem);
    }
    return { incidents, warnings, blocked };
  }

  computeHardStopReached(source, observed, projected, policyAmount) {
    if (source === 'preflight') {
      return observed >= policyAmount || projected > policyAmount;
    }
    return projected >= policyAmount;
  }

  buildWarningEntry(policy, projected, warnLimit, ts, source, details) {
    if (!policy.notifyEnabled) return null;
    if (projected < warnLimit) return null;
    ...
    return { ...policy, observedAmount: projected, incident };
  }

  buildHardStopEntry(policy, projected, ts, source, details) {
    const incident = this.ensureIncident({
      policy,
    ...
    return {
      incident,
      blockedItem: { ...policy, observedAmount: projected, incident: incident || null },
    };
  }
