...
// P5（2026-07-02）：维度不等此前静默返 0——embedding 模型/维度切换后语义召回整体退化为零命中却无任何
//   线索（16 病灶#9）。告警一次（防日志刷屏），把「为什么召回突然全靠 FTS」变成可诊断。
...
function cosineSim(a, b) {
  if (!Array.isArray(a) || !Array.isArray(b) || a.length === 0 || a.length !== b.length) {
    if (!dimMismatchWarned && Array.isArray(a) && Array.isArray(b) && a.length > 0 && b.length > 0 && a.length !== b.length) {
      dimMismatchWarned = true;
      console.warn(`[knowledge-store] embedding 维度不匹配 ${a.length}≠${b.length}：相似度按 0 计,语义召回将退化(疑似 embedding 模型/维度切换,建议重建索引)。本告警每进程只报一次`);
    }
    return 0;
  }
  let dot = 0, na = 0, nb = 0;
...
function tokenize(text) {
  ...
  const tokens = [];
  // 英文/数字词
  for (const m of text.toLowerCase().matchAll(/[a-z0-9]+/g)) if (m[0]) tokens.push(m[0]);
  // 中文字符（单字作为 token）
  for (const m of text.matchAll(/[一-鿿]/gu)) if (m[0]) tokens.push(m[0]);
  return tokens;
}

export class KnowledgeStore {
  constructor({ kbDir = KB_DIR } = {}) {
    this.kbDir = kbDir; // 测试可注入隔离目录；默认行为不变
    this._ensureDirs();
  }

  _ensureDirs() {
    if (!existsSync(DIR)) mkdirSync(DIR, { recursive: true, mode: 0o700 });
    if (!existsSync(this.kbDir)) mkdirSync(this.kbDir, { recursive: true, mode: 0o700 });
  }

  _kbDir(name) {
    return join(this.kbDir, name);
  }

  _readIndex(name) {
    const f = join(this._kbDir(name), 'index.json');
    // 强健工程：损坏时自动备份 .corrupted-*.bak（旧实现静默返 null，证据灭失且无从恢复）
    return readJsonWithCorruptBackup(f, { label: 'kb-index' }) || null;
  }

  _writeIndex(name, idx) {
    const f = join(this._kbDir(name), 'index.json');
    // 强健工程：原子写+一代备份（index.json 损坏 = 整个 KB 元数据不可访问）
    atomicWriteJson(f, idx);
  }
  ...
  list() {
    let entries;
    try { entries = readdirSync(this.kbDir, { withFileTypes: true }); }
    catch { return []; }
    const out = [];
  ...
  delete(name) {
    ...
    if (!dir.startsWith(this.kbDir + '/')) return false;
