...
const ROLLBACK_RATIO_THRESHOLD = 0.6; // 回滚率超此值即报「算力浪费」（G6，即使有真保留也警示）
...
export function createMetaEvolution({
  ...
  function runOnce() {
    ...
    // G6（2026-07-03）：回滚率——rolledBack=applied:false 总数（改不动的逻辑被回滚 + 浅层被价值闸拒）。
    //   旧健康判据「有任何真保留就判健康」对大规模浪费盲（实测 68% 回滚仍报"健康"）。有 rolledBack 字段且
    //   回滚率超阈值时，即使有真保留也先报警「算力浪费」。字段缺失（旧数据源）→ ratio=null 不触发（向后兼容）。
    const rolledBack = Number(stats.rolledBack);
    const rollbackRatio = (Number.isFinite(rolledBack) && total > 0) ? rolledBack / total : null;

    // 诊断 → 建议（recommendation 永远是给 owner 的文字，不是可执行动作）。
    let advisory;
    if (rollbackRatio != null && rollbackRatio > ROLLBACK_RATIO_THRESHOLD) {
      // 高回滚率优先报警：即使有真保留，大量算力在改不动/浅层目标上空转，是策略层该知道的浪费。
      advisory = {
        title: '元进化诊断：回滚率过高（大量算力浪费）',
        body: `近 ${total} 次进化里 ${rolledBack} 次被回滚/拒（${Math.round(rollbackRatio * 100)}%），仅 ${logicChanged} 次真保留改逻辑 + ${testOnly} 次补测试留存。虽有真价值产出，但过半算力花在改不动或浅层的目标上。`,
        severity: 'warn',
        tags: ['meta_evolution', 'advisory', 'high_rollback'],
        recommendation: '建议 owner：①降低高 drop 信号（如 test_gap）权重、把算力转投高保留率信号（self_directed/type_error）；②对反复回滚的同一目标文件加长冷却，别锁死空转；③检查 implement 能力是否匹配当前重构目标难度（本地/云端）。',
      };
    } else if (logicChanged > 0 || testOnly > 0) {
