...
export class ActPipeline {
  ...
  async #executeReal(actId, act, input = {}, evidenceContext = {}) {
    ...
    try {
      ...
      if (SE) {
        selfEvolution = {};
        for (const k of ['code', 'needsSelfRepair', 'needsConsensus', 'applyReportRef', 'runtimeReportRef', 'rollbackReportRef', 'priorApplyReportRef', 'priorRollbackRef', 'rolledBack', 'skipped']) {
  ...
  async retry(actId, input = {}) {
    ...
    if (!approvedExecuteRetry && !['failed', 'cancelled', 'blocked_safety'].includes(current.status)) {
