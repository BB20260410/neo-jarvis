...
import { shouldDecompose, buildDecompositionInstruction } from './NoeTargetDecomposition.js';
import {
  resolveSelfEvolutionImplementerCandidates,
  objectiveHasPriorRepairEvidence,
} from './NoeSelfEvolutionCandidateOrder.js';
import { improveSignalFromVerifyFailure } from '../room/NoeSelfEvolutionImproveSignal.js';
...
async function applyAndVerify({ root, patchPlanRef, runtimeVerify, now, applyReportsDir, skipDedup = false, evolutionOutcome = null, evolutionLogicGate = null, appendEvent = null }) {
  ...
  if (!dryRun.ok) {
    const preflightBlockers = (dryRun.blocked || []).flatMap((b) => b.blockers || []).slice(0, 20);
    // P2 观测（2026-07-02）：preflight 被拦此前只随 act 失败信息流走，events 查不到 reportRef/确切 blocker
    //   （07-01 排查曾因此多轮找不到根因）。落独立事件让「为什么被拦」可直接归因。fail-open。
    if (typeof appendEvent === 'function') {
      try {
        appendEvent({
          kind: 'self_evolution_preflight_blocked',
          ts: Date.now(),
          tag: 'noe.self_evolution.preflight_blocked',
          blockers: preflightBlockers,
          reportRef: clean(dryRun.reportRef || '', 500),
          patchPlanRef: clean(patchPlanRef, 500),
          secretValuesReturned: false,
        });
      } catch { /* fail-open：观测事件失败不阻断 */ }
    }
  ...
  // P2（2026-07-02）：reason 记拦截点归因——此前 66% 回滚在 evolution_outcome 里分不清 verify 不绿/双绿门拒/
  //   补测试无效/护栏拦，故障日（如 6-30 全天 93% 回滚）无法定位。
  const recordOutcomeFinal = (kept, reason = '') => {
    if (evolutionOutcome && outcomeBefore && outcomeAfter) {
      try { evolutionOutcome.record({ patchPlanId: readPatchPlanIdFromRef(root, patchPlanRef), before: outcomeBefore, after: outcomeAfter, applied: kept, reason }); } catch { /* fail-open */ }
    }
  };
  ...
  if (mutatedProtected.length) {
    ...
    recordOutcomeFinal(false, 'protected_file_mutated_post_apply');
  ...
  if (evolutionLogicGate && outcomeSummary) {
    ...
    if (pre.block) {
      const rolledBack = runNoePatchRollback({ root, applyReportRef: applied.reportRef, dryRun: false, confirmOwner: true, now });
      recordOutcomeFinal(false, `logic_gate_pre:${pre.reason || 'blocked'}`);
  ...
  if (shouldRollbackVerify({ verify, baselineFailedTests, relativeEnabled: process.env.NOE_EVOLUTION_RELATIVE_BASELINE === '1' })) {
    ...
    const rolledBack = runNoePatchRollback({ root, applyReportRef: applied.reportRef, dryRun: false, confirmOwner: true, now });
    recordOutcomeFinal(false, 'verify_not_green');
  ...
  if (addsTestFile && baselineTotalTests != null && typeof verify.numTotalTests === 'number'
    ...
    const rolledBack = runNoePatchRollback({ root, applyReportRef: applied.reportRef, dryRun: false, confirmOwner: true, now });
    recordOutcomeFinal(false, 'added_test_not_effective');
    return {
  ...
  if (evolutionLogicGate && outcomeSummary) {
    ...
    if (!post.allow) {
      const rolledBack = runNoePatchRollback({ root, applyReportRef: applied.reportRef, dryRun: false, confirmOwner: true, now });
      recordOutcomeFinal(false, `logic_gate_post:${post.reason || 'blocked'}`);
      throw selfEvolutionError('self_evolution_logic_change_blocked', {
  ...
  recordOutcomeFinal(true, 'kept');
  return { ok: true, applied, verify };
}

async function resolvePatchPlanRef({ ctx, root, spawnImplementer, gate, escalate = false }) {
  ...
  // repair 升级模型链(2026-07-03)：escalate=true（self_repair 拍）透传给实施者——本地已败过一次且失败证据
  //   已拼进 objective(A2)，工厂按 repairEscalate flag 决定云端优先（实证本地 27b 修不动/作弊被锚拦）。
  const out = await spawnImplementer({ objective: clean(ctx.objective || ctx.goal || '', 1000), targetFile: ctx.targetFile || '', root, gate, ...(escalate ? { escalate: true } : {}) });
...
export function registerNoeSelfEvolutionExecutors(executors, deps = {}) {
  ...
  executors.set('noe.self_evolution.implementation', async ({ act }) => {
    ...
    const outcome = await applyAndVerify({ root, patchPlanRef, runtimeVerify: effectiveVerify, now: asDate(now), applyReportsDir, evolutionOutcome, evolutionLogicGate, appendEvent });
    ...
    if (outcome.skipped) {
      ...
      if (!outcome.ok) {
        const verifyReason = clean((outcome.verify && (outcome.verify.reason || outcome.verify.error)) || outcome.reason || '', 300);
        // Perception ring: structured ImproveSignal on verify-fail (anchor for self_repair / lessons).
        const improveSignal = improveSignalFromVerifyFailure({
          targetFile: ctx.targetFile || '',
          verifyReason,
          objective: ctx.objective || ctx.goal || '',
          errorClass: outcome.reason || 'verify_not_green',
        });
        throw selfEvolutionError('self_evolution_verify_failed_rolled_back_needs_self_repair', {
          ...
          // A2 失败证据回灌：verify 失败的「原因」此前随 throw 丢失 → self_repair 盲重试。挂结构化字段
          //   经 ActPipeline 白名单透传，trigger（flag NOE_SELFEVO_REPAIR_HINTS）存 cycle.repairHints。
          verifyReason,
          improveSignal,
        });
      }
    ...
    if (!outcome.ok) {
      ...
      throw selfEvolutionError('self_evolution_verify_failed_rolled_back_needs_self_repair', {
        ...
        // A2 失败证据回灌：把 verify 失败原因（type 价值锚 reason / 补测试无效 reason / 异常 error）带给
        //   trigger，self_repair 的 implementer 才知道「上次为什么不过」（治 359 次盲重试）。
        verifyReason,
        // Perception ring: technical anchor for memory/self_repair (not a parallel engine).
        improveSignal,
      });
    }
  ...
  executors.set('noe.self_evolution.self_repair', async ({ act }) => {
    ...
    const patchPlanRef = await resolvePatchPlanRef({ ctx, root, spawnImplementer, gate, escalate: true });
    // C 锚旁路修复(2026-07-03)：self_repair 此前用裸 runtimeVerify（无 type 价值锚）——type_error goal 的 repair
    //   只要 npm test 绿就算成功，没修 error 也盖章 = 假进化虚高「真进步率」。与 implementation 同判据包装；
    //   守卫同款：仅 ctx.signal==='type_error' 且注入了 typeErrorVerify 才包装（其他 goal / flag OFF 零回归）。
    let repairVerify = runtimeVerify;
    if (ctx.signal === 'type_error' && ctx.targetFile && typeof typeErrorVerify === 'function') {
      repairVerify = typeErrorVerify({ baseVerify: runtimeVerify, targetFile: ctx.targetFile, beforeErrorCount: ctx.beforeErrorCount, root });
    }
    ...
    const outcome = await applyAndVerify({ root, patchPlanRef, runtimeVerify: repairVerify, now: asDate(now), applyReportsDir, skipDedup: true, evolutionOutcome, evolutionLogicGate, appendEvent });
    if (!outcome.ok) {
...
// 阶段二B（2026-07-03）：检测「编造的 from」——replace op 的 from 在目标文件内容里逐字找不到(本地模型硬编造,
//   题库实测 fuzzy 都救不了)。返回编造的 from 列表,供 spawnImplementer 一次纠正 re-ask。content 为空则不误判(返 [])。纯函数。
export function findFabricatedReplaceFroms(operations, content) {
  const ops = Array.isArray(operations) ? operations : [];
  const src = String(content || '');
  if (!src) return []; // 无参照内容不误判(截断/多文件场景 fail-open)
  const out = [];
  for (const op of ops) {
    if (op && op.op === 'replace' && typeof op.from === 'string' && op.from) {
      if (!src.includes(op.from)) out.push(op.from);
    }
  }
  return out;
}

export function makeNoeSelfEvolutionImplementer({ getAdapter, route, root = process.cwd(), now = () => new Date(), structuredCall = noeStructuredCall, localFirst = false, localCodeAdapterId = 'lmstudio', fromReask = process.env.NOE_SELFEVO_FROM_REASK === '1', kgContext = () => '', decompose = process.env.NOE_SELFEVO_DECOMPOSE === '1', repairEscalate = process.env.NOE_SELFEVO_REPAIR_ESCALATE === '1' } = {}) {
  return async function spawnImplementer({ objective = '', targetFile = '', root: callRoot = root, escalate = false } = {}) {
    ...
    // 阶段二·记忆图谱参与推理（NOE_SELFEVO_KG_REASONING,默认注入 ()=>'' 零回归）：附「图谱里关于本目标的关联知识」,
    //   让模型带着已知模块关联去改,而不是孤立看文件。fail-open:kgContext 抛错也不阻断。
    let kg = '';
    try { kg = String(kgContext(obj) || ''); } catch { kg = ''; }
    // 阶段二·难目标分解（NOE_SELFEVO_DECOMPOSE,默认 OFF 零回归）：复杂目标(文件长/多子句)附「只做最小第一步」约束——
    //   小改动=小 from=能逐字复制,治本地模型编造 from 的根因;剩余部分留后续迭代。fail-open。
    let decomp = '';
    try { if (decompose && shouldDecompose({ objective: obj, fileContent: fileContext }).decompose) decomp = buildDecompositionInstruction(obj); } catch { decomp = ''; }
    const userContent = (fileContext
      ? `Objective: ${obj}${targetLine}\n\n目标文件的真实当前内容（"from" 必须从下面逐字复制，严禁编造文件里不存在的片段）：\n${fileContext}\n\nProduce the noe_patch_plan now.`
      : `Objective: ${obj}${targetLine}\nProduce the noe_patch_plan now.`) + kg + decomp;
    ...
    // 任务2（2026-07-03）：本地 code adapter 可配（localCodeAdapterId，默认 'lmstudio' 零回归）。
    //   实测:本地模型(35b/27b)产结构化 patch(JSDoc/签名)from 精确命中、复杂内部逻辑会编造 from → 配 fuzzy 容错 +
    //   云端第 2 候选兜底。给 implement 单独接 temp 0 的专用本地 code adapter(如 lmstudio-code=27b),比借主脑(temp 0.2)准。
    const localId = localCodeAdapterId || 'lmstudio';
    // repair 升级模型链：escalate + flag → 云端真优先（不得把 lmstudio-code 放首位）。
    // 2026-07-18 修复：旧逻辑 [decision.adapterId, local] 在 LOCAL_FIRST 下 adapterId 仍是本地，
    //   「有错误 patch」不会级联 → escalate 实测≈0。现用 resolveSelfEvolutionImplementerCandidates。
    const hasPrior = objectiveHasPriorRepairEvidence(obj);
    const cloudFirstForRepair = (repairEscalate && escalate === true) || (escalate === true && hasPrior);
    const candidateIds = resolveSelfEvolutionImplementerCandidates({
      routeAdapterId: decision.adapterId || null,
      localCodeAdapterId: localId,
      localFirst: localFirst && !cloudFirstForRepair,
      cloudFirst: cloudFirstForRepair,
      hasPriorRepairEvidence: hasPrior && escalate === true,
    });
    ...
    for (const aid of candidateIds) {
      ...
      let candAttempts = 0;
      let candError = '';
      let correction = ''; // 阶段二B：检测到编造 from 时的纠正提示,附到下一 attempt 重问
      for (let attempt = 0; attempt < MAX_ATTEMPTS_PER_ADAPTER; attempt += 1) {
        ...
        if (result && result.ok) {
          ...
          if (isUsablePatchPlan(p)) {
            // 阶段二B（NOE_SELFEVO_FROM_REASK,默认OFF零回归）：检测编造 from,若有且还有 attempt → 附纠正提示 re-ask
            //   一次(治本地模型硬编造 from、fuzzy 都救不了→提落地率)。末次 attempt 仍编造则接受(下游 fuzzy/apply 兜底)。
            const fabricated = fromReask ? findFabricatedReplaceFroms(p.operations, fileContext) : [];
            if (fabricated.length && attempt < MAX_ATTEMPTS_PER_ADAPTER - 1) {
              correction = `你上次输出的这些 "from" 在目标文件里逐字找不到(编造了): ${fabricated.map((f) => JSON.stringify(String(f).slice(0, 60))).join(', ')}。请严格只从上面「目标文件的真实当前内容」里逐字复制 from(含缩进);找不到确切文本就改用 op:write_file 全量重写,绝不要猜 from。`;
              candError = 'fabricated_from_reask';
              continue;
            }
            candPlan = p; candError = ''; break;
          } // 出可用 patch（operations 非空）即停
...
export function makeNoeSelfEvolutionRuntimeVerify({ cwd = process.cwd(), spawnFn = null, now = () => new Date(), readJsonReport = null } = {}) {
  ...
  return async function runtimeVerify({ root: verifyRoot = cwd } = {}) {
    ...
    // D 可观测(2026-07-03)：挂测「名字」此前读完即弃（下方 finally unlink 中间 JSON）——实证 119/200 报告恰 1 挂测
    //   拖死 baseline/verify（逻辑门 baseline_not_green 全拒）却无法归因是哪个测试。持久化 ≤10 条 fullName（截断）。
    let failedTests = [];
    try {
      const parsed = readReport(jsonPath);
      try {
        failedTests = (Array.isArray(parsed && parsed.testResults) ? parsed.testResults : [])
          .flatMap((tr) => (Array.isArray(tr && tr.assertionResults) ? tr.assertionResults : []))
          .filter((a) => a && a.status !== 'passed' && a.status !== 'skipped' && a.status !== 'pending' && a.status !== 'todo')
          .map((a) => clean(a.fullName || a.title || '', 300))
          .filter(Boolean)
          .slice(0, 10);
      } catch { failedTests = []; /* fail-open：名单提取失败只影响可观测，不影响 ok 判定 */ }
    ...
    try { chmodSync(file, 0o600); } catch { /* best-effort */ }
    return { ok, exitCode, reportRef: ref, error, numTotalTests, numFailedTests, reportTrusted, reportError, failedTests };
  };
}
