...
/**
 * Rotates a JSONL log file if it exceeds a size threshold.
 *
 * If the file exists and its size is greater than or equal to `maxBytes`,
 * it is renamed to `${file}.1` (overwriting any existing `.1` file).
 * This keeps the active log file small for efficient tailing.
 *
 * @param {string} file - The path to the JSONL file.
 * @param {number} maxBytes - The maximum size in bytes before rotation.
 * @param {object} [fs] - Optional filesystem interface for testing.
 * @returns {boolean} True if the file was rotated, false otherwise.
 */
export function rotateJsonlIfLarge(file, maxBytes, fs = { existsSync, statSync, renameSync }) {
  try {
    if (maxBytes > 0 && fs.existsSync(file) && fs.statSync(file).size >= maxBytes) {
...
function pickRuleId(input) {
  if (typeof input.id === 'string' && input.id) return input.id.slice(0, 60);
  return 'rule-' + randomUUID().slice(0, 8);
}

function pickRuleName(input) {
  if (typeof input.name !== 'string') return null;
  return input.name.slice(0, MAX_RULE_NAME).trim() || null;
}
...
function pickRuleMode(value) {
  return (value && VALID_MODES.includes(value)) ? value : null;
}

function pickRuleBool(input, key) {
  return input[key] !== false;
}

function sanitizeRule(input) {
  if (!input || typeof input !== 'object') return null;
  const id = pickRuleId(input);
  const name = pickRuleName(input);
  if (!name) return null;
  const when = pickFromList(input.when, VALID_EVENTS, null);
  if (!when) return null;
  const action = pickFromList(input.action, VALID_ACTIONS, 'noop');
  const sourceMode = pickRuleMode(input.sourceMode);
  const targetMode = pickRuleMode(input.targetMode);
  if (action === 'forward' && !targetMode) return null;
  return {
    id, name, when, action, sourceMode, targetMode,
    enabled: pickRuleBool(input, 'enabled'),
    autoStart: pickRuleBool(input, 'autoStart'),
  };
}
...
export class AutopilotStore {
  ...
  _load() {
    // 审计 §续审 P1：损坏文件先备份 .corrupted-*.bak 再返 null（证据不灭失），null 时保持构造默认
    const data = readJsonWithCorruptBackup(FILE, { label: 'autopilot' });
    if (!data) return;
    try {
  ...
  _save() {
    try {
      // 审计 §续审 P1：原子写（tmp+rename）+ 覆盖前备份——原 writeFileSync 直接覆盖，写一半被 kill
      // 会损坏 autopilot.json 致用户规则丢失（atomicJsonFile 正是为此建，6 个 Store 已迁，此处补齐）。
      atomicWriteJson(FILE, this.config, { mode: 0o600 });
    } catch (e) {
  ...
  _rotateLogIfLarge() {
    const maxBytes = Number(process.env.NOE_AUTOPILOT_LOG_MAX_BYTES) || 5 * 1024 * 1024;
    rotateJsonlIfLarge(LOG_FILE, maxBytes);
  }

  /** 构建 activity log 记录并安全写入（抽离以降低 log 的圈复杂度） */
  _recordLogActivity(entry) {
