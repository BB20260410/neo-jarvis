...
function resolveJobStatus(input) {
  return JOB_STATUSES.has(input.status) ? input.status : 'queued';
}

function resolveJobAction(input, schedule) {
  return sanitizeAction(input.action ?? schedule?.action);
}

function resolveJobTargetField(field, input, schedule, max = 512) {
  return toNullableString(input[field] ?? schedule?.[field], max);
}

function resolveJobTargetContext(input, schedule) {
  return {
    targetType: resolveJobTargetField('targetType', input, schedule, 80),
    targetId: resolveJobTargetField('targetId', input, schedule),
    roomId: resolveJobTargetField('roomId', input, schedule),
    sessionId: resolveJobTargetField('sessionId', input, schedule),
    taskId: resolveJobTargetField('taskId', input, schedule),
    projectId: resolveJobTargetField('projectId', input, schedule),
  };
}

function resolveJobMaxRetries(input, schedule) {
  return input.maxRetries ?? schedule?.maxRetries ?? 2;
}

function resolveJobRetryBackoff(input, schedule) {
  return toInt(input.retryBackoffMs ?? schedule?.retryBackoffMs, schedule?.retryBackoffMs || 60_000, 1_000, 24 * 60 * 60 * 1000);
}

function resolveJobLimits(input, schedule, at) {
  return {
    priority: toInt(input.priority, 0, -1000, 1000),
    runAfter: normalizeTs(input.runAfter, at),
    attempts: toInt(input.attempts, 0, 0, 10_000),
    maxAttempts: toInt(input.maxAttempts, Math.max(1, Number(resolveJobMaxRetries(input, schedule)) + 1), 1, 100),
    retryBackoffMs: resolveJobRetryBackoff(input, schedule),
  };
}

function resolveJobPayload(input, schedule) {
  return input.payload !== undefined ? input.payload : (schedule?.payload || {});
}
...
function sanitizeJobInput(input = {}, schedule = null) {
  ...
  return {
    ...
    status: resolveJobStatus(input),
    action: resolveJobAction(input, schedule),
    ...resolveJobTargetContext(input, schedule),
    ...resolveJobLimits(input, schedule, at),
    dedupeKey: toNullableString(input.dedupeKey, 512),
    payload: resolveJobPayload(input, schedule),
  };
}
...
/**
 * Calculates the next scheduled run time for a recurring schedule.
 *
 * Uses a mathematical step calculation to avoid performance issues with large time gaps
 * (R2-P1 fix). Returns null for 'once' schedules.
 *
 * @param {object} schedule - The schedule object containing kind, interval, and nextRunAt.
 * @param {number} referenceTs - The current timestamp or reference point in milliseconds.
 * @returns {number|null} The next run timestamp in milliseconds, or null if the schedule is 'once'.
 */
export function nextScheduleRun(schedule, referenceTs) {
  ...
  const base = Number(schedule.nextRunAt) || referenceTs;
  // R2-P1（2026-07-03）：一步跳算式替代无上界 do-while。旧循环在停机数天 + interval=1s 时会迭代
  //   （referenceTs-base)/interval ≈ 百万次，同步阻塞事件循环（且在 db.transaction 内）。数学等价：
  //   跳到第一个 > referenceTs 的对齐点。base 已 > referenceTs 时直接返回 base（跳 0 步）。
  if (base > referenceTs) return base;
  const steps = Math.floor((referenceTs - base) / interval) + 1;
