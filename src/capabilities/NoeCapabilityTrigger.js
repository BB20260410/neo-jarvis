...
export function createNoeCapabilityTrigger(deps = {}) {
  const {
    ...
    // 阶段二·扩展工具生态:装新工具前先查现有已注册工具能否满足(need)=>匹配的 enabled 工具[]。默认 null=不查(零回归)。
    matchExistingTools = null,
  } = deps;
  ...
  async function observe({ text = '', need = '', kind = 'any' } = {}) {
    ...
    // 阶段二·扩展工具生态:先查现有工具能否满足——有 enabled 现成的就用,不重复搜索/安装(更好用现有生态 + 零新增装机风险)。
    if (typeof matchExistingTools === 'function') {
      let existing = [];
      try { existing = matchExistingTools(q) || []; } catch { existing = []; }
      if (Array.isArray(existing) && existing.length) {
        return { ok: true, alreadyAvailable: true, tool: existing[0], candidates: existing, reason: 'existing_tool_matches', need: q };
      }
    }
  ...
  // 轴3（2026-07-03）：拓宽信号源——从一批文本（goal 标题+why、失败/学习教训、自我对话）里挑第一条表达能力缺口的。
  //   治「capabilityTick 只扫 goal 标题→自进化 goal 标题都是『改进X/补测试』永不匹配→0 落地」：能力缺口更常出现在
  //   goal 的 why 或 Neo 反思失败时的教训里，不在终 title。命中 → 交 observe 走完整安全链（搜/评估/grant/提议，不自动装）。
  //   纯函数、fail-open。返回匹配的文本（供 observe 的 need），无则 null。
  function scanForCapabilityNeed(texts = []) {
    if (!Array.isArray(texts)) return null;
    for (const raw of texts) {
      const t = cleanStr(raw);
      if (t && classifyCapabilityNeed(t).isNeed) return t.slice(0, 200);
    }
    return null;
  }

  return { observe, classifyCapabilityNeed, scanForCapabilityNeed };
}
