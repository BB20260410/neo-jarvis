...
export const DANGER_RULES = [
  // ===== CRITICAL（必拦） =====
  // L8 修复：允许引号包裹根路径（rm -rf "/" 或 '/'），原正则只匹配裸 / ~ $HOME，被引号绕过。
  ...
  // 2026-06-11 审计补丁：混淆/间接执行绕过（实测 base64|sh、wget|sh、下载后执行均穿透旧规则）
  { pattern: /base64\b.*\|\s*(bash|sh|zsh)(\s|$)/, severity: 'critical', category: 'base64 解码后执行', advice: '解码内容未经审计直接进 shell，典型混淆执行' },
  { pattern: /wget\s+.*\|\s*(bash|sh|zsh)(\s|$)/, severity: 'critical', category: '远程脚本直接执行（wget）', advice: '先下载到文件审计再执行' },
  { pattern: /\brm\b[^|;&]*--no-preserve-root/, severity: 'critical', category: 'rm 解除根目录保护', advice: '--no-preserve-root 专用于删除根目录，禁止' },
  { pattern: /(>|>>)\s*\/dev\/(sd[a-z]|hd[a-z]|nvme\d\S*|r?disk\d\S*)/, severity: 'critical', category: '重定向覆写块设备', advice: '直接写块设备会破坏磁盘数据' },
  ...
  { pattern: /dd\s+if=\/dev\/(zero|u?random)\s+of=\/dev\/(sd|hd|nvme|r?disk)/, severity: 'critical', category: '硬盘清零', advice: '会清空真实磁盘' },

  // ===== HIGH（默认拦） =====
  // v0.17 真测发现的盲区补丁：rm -rf 任意绝对路径（除明显安全的临时目录显式白名单）
  { pattern: /\brm\s+-[a-zA-Z]*[rR][a-zA-Z]*\s+\/(?!tmp\/|var\/folders\/|private\/tmp\/)\S+/, severity: 'high', category: '递归删除绝对路径', advice: '会不可恢复，请先 ls 确认目标，或用 trash 命令' },
  { pattern: /\brm\s+-[a-zA-Z]*[rR][a-zA-Z]*\s+~\//, severity: 'high', category: '递归删除家目录子路径', advice: '~/ 是用户家目录，谨慎删除子路径，先 ls 确认' },
  { pattern: /\brm\s+-[a-zA-Z]*[rR][a-zA-Z]*\s+(\.\.?\/|\.\.\/?)/, severity: 'high', category: '递归删除上级/当前目录', advice: '改用具体子路径，避免 .. 含义不明' },
  ...
  // 2026-06-11 审计补丁：下载落盘后执行、eval 动态求值
  { pattern: /curl\b[^|;&]*-[a-zA-Z]*o\s+\S+.*&&\s*(bash|sh|zsh)\b/, severity: 'high', category: '下载后立即执行', advice: '下载与执行拆开，中间人工审计脚本内容' },
  { pattern: /\beval\s*["']?[$`]/, severity: 'high', category: 'eval 动态求值', advice: 'eval 变量/命令替换内容不可审计，改直接调用' },
