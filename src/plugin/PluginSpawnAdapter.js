...
import { terminateNoeProcessTree } from '../runtime/NoeProcessTree.js';
...
export class PluginSpawnAdapter {
  ...
  async execCommand(commandId, params = {}, opts = {}) {
    ...
    const allowSecretEnv = this.manifest.sandbox?.allowSecretEnv === true || opts.allowSecretEnv === true;
    const env = sanitizeNoeHostExecEnv(process.env, {
      allowlist: envWhitelist,
      allowSecrets: allowSecretEnv,
    });
    // 强制 UTF-8
    if (!env.LANG) env.LANG = 'zh_CN.UTF-8';
    if (!env.LC_ALL) env.LC_ALL = 'zh_CN.UTF-8';
    if (opts.env && typeof opts.env === 'object') {
      Object.assign(env, sanitizeNoeHostExecEnv(opts.env, {
        allowlist: Object.keys(opts.env),
        allowSecrets: allowSecretEnv,
      }));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const terminateTree = () => {
        if (child?.pid) terminateNoeProcessTree(child.pid, { graceMs: 2000 }).catch(() => {});
        else {
      ...
      // M9 修复：用 'close' 而非 'exit' 收集输出——'exit' 时 stdout/stderr 管道可能尚未读完会截断 reply；
      // 'close' 保证所有 stdio 流结束后再读。
      child.on('close', (code) => {
