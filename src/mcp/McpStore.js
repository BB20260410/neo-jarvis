...
export class McpStore {
  ...
  _load() {
    const data = readJsonWithCorruptBackup(this.file, { label: 'mcp' });
    if (!data) return; // 不存在或损坏（损坏已自动备份 .corrupted-*.bak）→ 保持空列表
    try {
  ...
  _save() {
    try {
      // 强健工程：原子写+一代备份（旧实现直接覆盖正式文件，写一半被 kill = MCP 配置全损无备份）
      atomicWriteJson(this.file, { version: 1, servers: this.servers });
    } catch (e) {
