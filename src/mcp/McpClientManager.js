...
export class McpClientManager {
  constructor({ store, baseEnv = process.env } = {}) {
  ...
  async ensureConnected(name) {
    ...
    let transport;
    if (cfg.type === 'stdio') {
      // 合并基础环境（生产默认 process.env，smoke 可注入最小环境）+ 用户 env
      const env = { ...this.baseEnv, ...(cfg.env || {}) };
    ...
    const timeoutP = new Promise((_, rej) => { connectTimer = setTimeout(() => rej(new Error('connect timeout')), CONNECT_TIMEOUT_MS); });
    try {
      ...
      // L6 修复：连接成功/失败后都清掉超时计时器，否则成功后它仍空挂 CONNECT_TIMEOUT_MS。
  ...
  async callTool(name, toolName, args = {}) {
    ...
    const t0 = Date.now();
    try {
      // H3 修复：用 SDK 的 per-request timeout，超时只取消本次请求（发 cancelled 通知）并 reject，
      // 不再 disconnect 共享连接——避免误杀同一 server 上并发 in-flight 的其它 callTool 与共享子进程。
      const out = await entry.client.callTool({ name: toolName, arguments: args }, undefined, { timeout: CALL_TOOL_TIMEOUT_MS });
