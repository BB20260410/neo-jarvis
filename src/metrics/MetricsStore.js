...
export class MetricsStore {
  ...
  _warmCache() {
    ...
    // M12 修复：50MB 滚动后旧数据改名为 metrics-<月>.jsonl.<ts>，只读当前 .jsonl 会漏掉当月滚动归档；
    // 读该月所有文件（归档按 ts 升序在前、当前 .jsonl 排最后）再取 tail。
    let files;
    try {
      files = readdirSync(this.dir)
        .filter((f) => new RegExp(`^metrics-${month}\\.jsonl(\\.\\d+)?$`).test(f))
        .sort((a, b) => {
          const ma = a.match(/\.jsonl\.(\d+)$/); const mb = b.match(/\.jsonl\.(\d+)$/);
          return (ma ? Number(ma[1]) : Infinity) - (mb ? Number(mb[1]) : Infinity);
        });
    } catch { return; }
    if (!files.length) return;
    try {
      const lines = files.flatMap((f) => {
        try { return readFileSync(join(this.dir, f), 'utf-8').split('\n').filter(Boolean); } catch { return []; }
      });
  ...
  _readRange(fromTs, toTs) {
    ...
    let files;
    try {
      // P2 修复（2026-07-02）：兼容 50MB 滚动归档名 metrics-<月>.jsonl.<ts>（M12 只修了 _warmCache，
      //   这里漏了 → 历史区间查询把滚动过的月份少算一大截）。排序：月份升序，同月归档(ts 升序)在前、当前 .jsonl 最后。
      files = readdirSync(this.dir).filter((f) => /^metrics-\d{4}-\d{2}\.jsonl(\.\d+)?$/.test(f));
    } catch {
      return [];
    }
    files.sort((a, b) => {
      const monthA = a.slice(8, 15); const monthB = b.slice(8, 15);
      if (monthA !== monthB) return monthA < monthB ? -1 : 1;
      const ma = a.match(/\.jsonl\.(\d+)$/); const mb = b.match(/\.jsonl\.(\d+)$/);
      return (ma ? Number(ma[1]) : Infinity) - (mb ? Number(mb[1]) : Infinity);
    });
    for (const f of files) {
      const m = f.match(/^metrics-(\d{4}-\d{2})\.jsonl(\.\d+)?$/);
      if (!m) continue;
      const month = m[1];
