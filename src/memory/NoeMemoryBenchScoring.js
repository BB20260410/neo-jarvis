 ...
 * @param {BenchExpectation|null|undefined} expectation
 * @returns {string|null} 错误码或 null
 */
export function caseSchemaError(expectation) {
  const exp = expectation || /** @type {BenchExpectation} */ ({ id: '' });
 ...
 * @param {BenchExpectation|null|undefined} exp
 * @returns {number}
 */
 ...
 * @param {BenchRun|null|undefined} run
 * @param {BenchExpectation|null|undefined} expectation
 * @returns {{ passed: boolean, runOk: boolean, schemaError: string|null, precision: number, recall: number, blockedIds: string[], selectedCount: number, maxSelected: number, overRecall: boolean }}
 */
export function scoreOneRun(run, expectation) {
  ...
  const maxSelected = exp.maxSelected == null ? defaultMaxSelected(exp) : Number(exp.maxSelected);
