...
export class MemoryCore {
  constructor({ storage = sqliteStore, logger = console, semanticIndex = null, dedupe = null, conflictPolicy = null, fisherRank = null, salienceFusion = null, kgRecall = null } = {}) {
    ...
    // KG 参与主召回（P5，NOE_KG_RECALL=1 默认 OFF）：图谱此前只进不出（KG_INGEST/RELATIONS 摄入活跃，
    // 主召回链零消费=白攒资产）。开启且注入 graph（对象或 thunk，server 组合根晚构造用 thunk）时，
    // recallFused 增加第三路「图谱扩展」：query 命中实体 → 1-hop 邻居实体名 → FTS 扩展召回 → 第三列喂 RRF。
    // 关闭/未注入/任何失败 → 召回行为与旧逐字一致（fail-open）。
    const kgEnv = (kgRecall && kgRecall.env) || env;
    this.kgRecall = {
      enabled: kgRecall?.enabled ?? (kgEnv.NOE_KG_RECALL === '1'),
      graph: kgRecall?.graph || null,
      maxEntities: kgRecall?.maxEntities ?? 4,
      maxNeighborTerms: kgRecall?.maxNeighborTerms ?? 8,
    };
  }

  // KG 扩展路（P5）：query → 图实体 → 1-hop 邻居实体名 → FTS 召回。任何失败返 []（不拖累主召回）。
  #graphRouteHits(query, { limit, projectId } = {}) {
    const graph = typeof this.kgRecall.graph === 'function' ? this.kgRecall.graph() : this.kgRecall.graph;
    if (!graph || typeof graph.search !== 'function') return [];
    const pid = projectId || 'noe';
    const found = graph.search({ q: query, projectId: pid, limit: this.kgRecall.maxEntities });
    const entities = (found && Array.isArray(found.entities)) ? found.entities : [];
    if (!entities.length) return [];
    const terms = new Set();
    for (const ent of entities) {
      if (typeof graph.oneHop !== 'function') break;
      let hop = null;
      try { hop = graph.oneHop({ id: ent.id, projectId: pid, limit: 10 }); } catch { hop = null; }
      for (const edge of (hop && Array.isArray(hop.edges)) ? hop.edges : []) {
        const name = safeString(edge && edge.name, 60);
        if (name) terms.add(name);
        if (terms.size >= this.kgRecall.maxNeighborTerms) break;
      }
      if (terms.size >= this.kgRecall.maxNeighborTerms) break;
    }
    if (!terms.size) return [];
    const hits = this.recall({ q: [...terms].join(' '), limit, bumpHits: false, ...(projectId ? { projectId } : {}) }) || [];
  ...
  async recallFused(input = {}) {
    ...
    const projectId = normalizeProject(input.projectId ?? input.project_id ?? input.project);
    // P5 图谱扩展第三路（flag OFF / 无 graph / 无命中 → graphHits=[]，下方行为与旧逐字一致）。
    let graphHits = [];
    if (this.kgRecall?.enabled && this.kgRecall.graph) {
      try {
        graphHits = this.#graphRouteHits(query, { limit: vectorPool, projectId });
      } catch (e) {
        this.logger?.warn?.('[noe-memory] KG 扩展路失败(忽略):', e?.message || e);
        graphHits = [];
      }
    }
    if ((!Array.isArray(vecHits) || !vecHits.length) && !graphHits.length) return this.#finishRecall(base, input);
    const rankLists = [];
    if (Array.isArray(vecHits) && vecHits.length) rankLists.push(vecHits.map((v) => ({ id: v.refId })));
    rankLists.push(base.map((m) => ({ id: m.id })));
    if (graphHits.length) rankLists.push(graphHits);
