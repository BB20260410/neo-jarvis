...
//
// 文件：~/.noe-panel/archive-config.json
//   {
//     version: 1,
...
const DEFAULT_ROOT = join(homedir(), 'Documents', 'noe-archive');
...
export class ArchiveStore {
  constructor({ configFile = CONFIG_FILE } = {}) {
    this.configFile = configFile; // 测试可注入隔离文件；默认行为不变
  ...
  _load() {
    const data = readJsonWithCorruptBackup(this.configFile, { label: 'archive' });
    if (!data) return; // 不存在或损坏（已自动备份）→ 保持默认配置
    try {
  ...
  _save() {
    try {
      // 强健工程：原子写+一代备份（rootPath 等用户配置损坏会找不到已归档文件）
