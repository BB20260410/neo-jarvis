...
const noeMemoryCore = new MemoryCore(noeMemSemantic ? { semanticIndex: noeMemSemantic, fisherRank: { enabled: process.env.NOE_MEMORY_FISHER_RANK === '1' } } : {});
...
// 感知三件套（波次6 接线）：启动采集一次本机环境(ssh/git/桌面/硬件,只读元数据)，缓存进聊天 systemPrompt
collectHostContext()
  .then((ctx) => { setCachedHostContextBlock(ctx.combined); console.log('[noe-host-context] 本机感知三件套已缓存进聊天上下文'); })
...
const noeSelfEvolutionCycleStore = process.env.NOE_SELF_EVOLUTION === '1'
  ? new NoeSelfEvolutionCycleStore({ projectId: 'noe' })
  : null;
...
const noeSelfEvolutionTrigger = (noeSelfEvolutionCycleStore && noeGoalSystem)
  ? createNoeSelfEvolutionTrigger({
      ...
      realApply: process.env.NOE_SELF_EVOLUTION_REAL_APPLY === '1', // A4 真执行闸：默认 OFF=dry-run；ON 才让 implementation/self_repair 真改代码（仍过 gate+standing-grant+PolicyFileGuard）
  ...
  : null;
if (noeSelfEvolutionTrigger) console.log(`[noe-self-evolution] 环2 触发器已装配（observe 自发起意 + tick 单 writer 推进 Cycle；心跳通电另需 NOE_HEARTBEAT=1；consensus 自驱 ${noeSelfEvolutionConsensusAutodrive ? 'ON' : 'OFF'}；complete 闭环自驱 ${noeSelfEvolutionCompletionAutodrive ? 'ON·post_review真复核能拒绝' : 'OFF'}；real-apply ${process.env.NOE_SELF_EVOLUTION_REAL_APPLY === '1' ? 'ON·会真改代码' : 'OFF·dry-run'}）`);
  ...
  ? createImprovementSignalSeed({
      ...
      recentlyAttempted: (type, file) => {
        try {
          ...
          const row = getDb().prepare("SELECT id FROM noe_goals WHERE source='self_evolution' AND created_at > ? AND json_extract(meta,'$.signal')=? AND json_extract(meta,'$.file')=? LIMIT 1").get(Date.now() - cooldownMs, type, file);
          return !!row;
        } catch { return false; }
      },
    })
      ...
      outcomeStats: () => {
        try {
          ...
          for (const r of rows) {
            ...
            if (r.v === 'doc_only') docOnly = n; else if (r.v === 'neutral') neutral = n; else if (r.v === 'logic_changed') logicChanged = n; else if (r.v === 'test_only') testOnly = n;
          }
          return { total, docOnly, neutral, logicChanged, testOnly };
        } catch { return { total: 0, docOnly: 0, neutral: 0, logicChanged: 0, testOnly: 0 }; }
      },
...
if (process.env.NOE_HEARTBEAT === '1') {
  ...
  if (noeCapabilityTrigger) {
    ...
    noeHeartbeat.register('capabilityTick', {
      ...
      run: async () => {
        let goals = [];
        try { goals = noeGoalSystem?.list?.() || []; } catch { /* goal 读不到不阻断心跳 */ }
        const needGoal = Array.isArray(goals)
          ? goals.find((g) => g && (g.status === 'open' || g.status === 'active') && noeCapabilityTrigger.classifyCapabilityNeed(g.title || '').isNeed)
          : null;
        if (!needGoal) return { skipped: 'no_capability_need_goal' };
        return noeCapabilityTrigger.observe({ need: needGoal.title });
      },
    });
  }
