...
function ensureGlobalWs() {
  ...
  try {
    ...
    globalWsState.ws = ws;
    ws.onopen = () => {
      const wasReconnect = globalWsState.reconnectAttempts > 0;
      globalWsState.reconnectAttempts = 0;
      // 重连成功：断线期间可能错过推送，通知订阅者补刷一次（各区自行判断是否需要刷新）
      if (wasReconnect) { for (const fn of globalWsSubscribers) { try { fn({ type: 'reconnected' }); } catch {} } }
    };
    ws.onmessage = (e) => {
      let msg;
    ...
    ws.onclose = () => {
      ...
      // 指数退避永续重连（800ms 起 2 倍递增，封顶 30s，不再永久放弃——生产重启超 45s 面板也能自愈）
      globalWsState.reconnectAttempts++;
      const delay = Math.min(30000, 800 * Math.pow(2, globalWsState.reconnectAttempts - 1));
