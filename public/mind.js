...
function ownerToken() {
  try {
    ...
    if (t) { sessionStorage.setItem('panel-owner-token', t); history.replaceState(null, '', u.pathname + u.hash); }
  } catch { /* 自举失败走存量 */ }
...
function renderCuriosityFunnel(d) {
  ...
  const rows = stages.map((s) => {
    ...
    return `<div class="funnel-row"><div class="funnel-meta"><span>${esc(s.label)}</span><b>${esc(s.count)}</b></div><div class="funnel-track"><i style="width:${w}%"></i></div><span class="funnel-pct">${s.ofPrev == null ? '' : esc(s.ofPrev) + '%'}</span></div>`;
  }).join('');
  const diag = Array.isArray(d.diagnostics) && d.diagnostics.length
    ? `<div class="funnel-diag">瓶颈：${esc((d.diagnostics[0].split(':')[1] || d.diagnostics[0]).trim())}</div>` : '';
...
function renderModelHealth(d) {
  ...
  const brainRow = (b) => `<div class="mh-row">${dot(b.loaded)}<span>${esc(b.label || b.role || '脑')}</span><em>${b.loaded ? '已就位' : '未加载'}</em></div>`;
  const emb = d.embedding || {};
  const embOk = Boolean(emb.provider) && emb.provider !== 'hash' && emb.provider !== 'unknown' && emb.degraded !== true;
  box.innerHTML = [
    `<div class="mh-row">${dot(d.ollama?.available)}<span>Ollama</span><em>${esc(d.ollama?.status || '—')}</em></div>`,
    `<div class="mh-row">${dot(d.lmstudio?.available)}<span>LM Studio</span><em>${esc(d.lmstudio?.status || '—')}</em></div>`,
    `<div class="mh-sub">三脑就位 ${esc(d.brainsReady ?? 0)}/3</div>`,
    brainRow(d.brains?.main || {}), brainRow(d.brains?.review || {}), brainRow(d.brains?.fallback || {}),
    `<div class="mh-row">${dot(embOk)}<span>embedding</span><em>${esc(emb.provider || '—')}${emb.dimension ? ' · ' + esc(emb.dimension) + '维' : ''}${emb.degraded ? ' · 降级' : ''}</em></div>`,
  ].join('');
}
...
// 轮询调度：页面隐藏时暂停（省资源 + 不空打服务器），恢复可见时补刷一次并重启，卸载时清理。
// 初始首发已由上面的 loadAll()/loadVitals() 完成，startPolling() 只装 interval 不立即执行，避免重复首发。
const POLL_JOBS = [
  { fn: loadAll, ms: 30_000, timer: 0 },
  { fn: loadWorkMap, ms: 15_000, timer: 0 },
  { fn: loadMissions, ms: 10_000, timer: 0 },
  { fn: loadWorldSurface, ms: 15_000, timer: 0 },
  { fn: loadVitals, ms: 5 * 60_000, timer: 0 },
];
let polling = false;
function startPolling() {
  if (polling) return;
  polling = true;
  for (const j of POLL_JOBS) j.timer = setInterval(j.fn, j.ms);
}
function stopPolling() {
  polling = false;
  for (const j of POLL_JOBS) { if (j.timer) { clearInterval(j.timer); j.timer = 0; } }
}
document.addEventListener('visibilitychange', () => {
  if (document.hidden) { stopPolling(); return; }
  // 恢复可见：走 loadAll/loadVitals 补全量（已覆盖 workMap/worldSurface/missions，避免各子任务重复请求）后重启轮询。
  loadAll();
  loadVitals();
  startPolling();
});
