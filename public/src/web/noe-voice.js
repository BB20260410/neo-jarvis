...
(function () {
  ...
  function startProactive() {
    ...
    const tick = async () => {
      ...
      try {
        ...
        const data = await res.json().catch(() => ({}));
        if (data.ok && data.spoke) { logVoice('Neo 贾维斯 · 主动陪伴', data.text || ''); if (data.audioBase64) playAudio(data.audioBase64, data.audioFormat); }
      } catch { /* 主动失败静默，不打扰 */ }
    };
    proactiveTimer = setInterval(tick, 60 * 1000); // 每 1 分钟轮询；是否真开口由后端冷却(NOE_PROACTIVE_COOLDOWN_MS,.env 已设1min)+变化检测决定
    logVoice('系统', '主动陪伴已开启（Neo 贾维斯会偶尔看一眼屏幕，只在值得时开口）');
  }
  ...
  async function endCaptureAndSend() {
    ...
    try {
      ...
      if (data.ok && data.audioBase64) { logVoice('Neo 贾维斯', data.reply || ''); vadPhase = 'speak'; speechFrames = 0; setState('🔊 ' + (data.usedAdapter || '') + '（说话可打断）'); pendingRest = null; if (data.restTtsText) fetchRestAudio(data.restTtsText, null); playReply(data.audioBase64, data.audioFormat); return; }
      if (data.ok) logVoice('Neo 贾维斯', data.reply || '');
