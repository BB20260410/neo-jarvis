...
npm start                     # 默认 127.0.0.1:51835（v0.49 N-01 起只听 localhost）
# 浏览器开 http://localhost:51835
```

**当前后台 PID**：`lsof -iTCP:51835 -sTCP:LISTEN -t` 看 PID。重启用 `kill -KILL <PID> && nohup node server.js > /tmp/panel-v052.log 2>&1 &`。

**数据位置**（持久化）：

| 文件 | 内容 | 权限 |
|---|---|---|
| `~/.noe-panel/data.json` | sessions + hookEvents + watcherHistory + starredIndices | 0o600 |
| `~/.noe-panel/rooms.json` | 房间（chat/debate/squad/**arena**）+ conversation + taskList + **archived/archivedAt/parentRoomId** | 0o600 |
| `~/.noe-panel/watcher.json` | watcher 配置 | 0o600 |
| `~/.noe-panel/prompts.json` | Prompt 模板库（⌘P 弹库）| 0o600 |
| `~/.noe-panel/room-adapters.json` | **v0.52**：room adapter 池（minimax/gemini/gemini-openai/gemini-cli/customs/spawn_overrides）+ timeoutMs + maxTokens | 0o600 |
| `~/.noe-panel/cli-plugins/*.json` | **v0.52**：用户 plugin manifest（builtin 在 src/plugin/builtin/）| 0o600 |
| `~/.noe-panel/metrics-YYYY-MM.jsonl` | **v0.53 Sprint 3**：按月切的 turn 指标流（每行一个 turn 摘要 + tokensIn/Out + estCostUSD）| 0o600 |
| `~/.noe-panel/room-templates.json` | **v0.53 Sprint 3**：用户保存的房间模板（builtin 6 个内置在 src/templates/RoomTemplatesStore.js）| 0o600 |
| `~/.noe-panel/webhooks.json` | **v0.54 Sprint 4**：webhook 配置（URL + 格式 + 事件订阅 + 触发 stats）| 0o600 |
| `~/.noe-panel/archive-config.json` | **v0.54 Sprint 4.5**：聊天归档配置（rootPath / structure / timeFormat / autoArchive）| 0o600 |
...
- 新模块 `src/archive/ArchiveStore.js`（323 行）：
  - 配置文件 `~/.noe-panel/archive-config.json`：`rootPath / structure / timeFormat / autoArchive / events`
...
### 4-1 Webhook 出站推送（🔔）
- 新模块 `src/webhook/WebhookStore.js`（持久化 ~/.noe-panel/webhooks.json，URL 必须 https:// 或 localhost http，最多 20 条）
...
for ep in 'search?q=x' 'prompts' 'sessions' 'plugins' 'room-adapters' 'rooms' 'ruflo/health' \
  ...
  curl -s -o /dev/null -w "  GET /api/$ep -> %{http_code}\n" "http://localhost:51835/api/$ep"
done
curl -s -o /dev/null -w "  /api/file → %{http_code} (want 403)\n" "http://localhost:51835/api/file?path=/etc/passwd"
curl -s -o /dev/null -w "  /api/browse → %{http_code} (want 403)\n" "http://localhost:51835/api/browse?path=/etc"

# 3. 看完成情况
浏览器 http://localhost:51835
- 顶栏 6 个按钮：📊 💻 💬 🐝 🧩 ⚙️ （v0.53 新加 📊）
- ⌘? 全部快捷键
- 进辩论房有「🔁 大轮数」+「🚀 启动辩论（N 大轮）」
...
- **PID**：执行 `lsof -iTCP:51835 -sTCP:LISTEN -t` 看
...
- **plugin 池**：默认 3 个 builtin（claude/codex/gemini-cli），用户可装到 `~/.noe-panel/cli-plugins/`
- **房间模板池**：默认 6 个 builtin（技术评审 / 事实对决 / 需求拆分 / 二方对辩 / 翻译对决 / Gemini 闲聊），用户可加自定义到 `~/.noe-panel/room-templates.json`，上限 50
