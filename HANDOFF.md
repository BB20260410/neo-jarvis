...
> **⏩ 2026-06-09 最新任务交接（先看这个）**：竞品分析(BaiLongma/OpenClaw/Hermes/MiniMax/Mavis)后的**未完成任务清单 + 其他 AI 怎么用** → 读 `docs/HANDOFF_竞品分析未完成任务_2026-06-09.md`。要接续这块直接说「继续任务」或点名读该文件。同会话刚做完：声纹 CAMPPlus+VAD、梦境系统接 server.js、自我认知接 VoiceSession。
...
外部依赖（不在本目录但项目需要）：
- `~/.noe-panel/data.json` — 持久化数据（sessions + 最近 200 条 msgs/session）
...
- [x] 持久化（`~/.noe-panel/data.json` 重启恢复）
...
- [x] **v0.33 Watcher Phase 1.2 Ollama Adapter（真测通过）**（2026-05-18 cycle_23）：
  ...
  - 写 `~/.noe-panel/watcher.json` provider=ollama / model=gemma3:4b / baseUrl=http://localhost:11434
...
- [x] **v0.32 Watcher Phase 1.1 框架 + MiniMax Adapter**（2026-05-18 cycle_22，用户跳序需求）：
  ...
  - `src/watcher/WatcherConfig.js`：`~/.noe-panel/watcher.json` 读写 + 深合并默认 + apiKey 600 权限 + 脱敏 helper
...
- [x] **v0.30 自查 bug 修复轮**（2026-05-18 08:50 cycle_21，用户「查问题修复优化完善」）：
  ...
  - mock 验证：GET /api/version 返回 `{ok:true, version:"0.29", appName:"noe-panel"}`
...
- 持久化：debounce 500ms 写 `~/.noe-panel/data.json`
...
3. 看 `~/.noe-panel/data.json` 当前已有 sessions
