...
3. 重启 server 看 `~/.noe-panel/data.json` 是否含这条 assistant 消息
...
- **N-01** server 默认监听 `*:51835`（暴露 LAN）→ 改 `127.0.0.1`，`PANEL_HOST=0.0.0.0` 可覆盖
...
- **审查 1**（语法 + 端到端）：8 个新加端点 200，重启 panel 在 51835 端口成功 v0.55
...
- **S12-N01** 新模块：`src/mcp/McpStore.js`（持久化 `~/.noe-panel/mcp-servers.json`，配置格式跟 Claude Desktop 兼容；含 stdio/sse/http 三类、enabled 开关、env / headers 掩码、上限 50 server）
