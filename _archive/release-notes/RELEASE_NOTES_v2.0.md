# Noe v2.0.0 — 数据底座 + 多 workspace + 向量搜索 + 结构化日志
...
- 自动扫描 ~/.noe-panel/ 下所有 mcp-calls-*.jsonl / metrics-*.jsonl / autopilot-log.jsonl / licenses-issued.jsonl
...
- `~/.noe-panel/workspaces/{name}/panel.db` (SQLite)
- `~/.noe-panel/workspaces/{name}/rooms.json` / `archive/` / etc
...
**输出位置**：`~/.noe-panel/logs/panel-YYYY-MM-DD.log` (0o600)
