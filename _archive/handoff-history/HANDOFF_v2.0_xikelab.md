...
git tag -l 'v*-noe' | tail    # 找最新 tag
curl http://127.0.0.1:51835/api/commercial/status | python3 -m json.tool  # 看商品化进度（panel 必须在跑）
```

如果 panel 没跑：`lsof -ti tcp:51835 || (PANEL_VERSION=2.0.0 nohup npm start > /tmp/panel.log 2>&1 &)`

---

## 🎯 项目是什么

**Noe**（息刻实验室）— 本地多 AI 工作台。
- 8+ AI provider（Claude/GPT/Gemini/MiniMax/Ollama 等）
- 4 种 AI 协作模式：chat / debate / squad / arena
- MCP 一站式 + Autopilot 自驾 + 向量语义搜索
- macOS Electron 桌面应用 + 浏览器 :51835 双模式
- 数据全本地 `~/.noe-panel/` 权限 0o600
...
| 改名史 | `v2.0.0-noe`（最新 tag）| Claude Panel → Roundtable → Hangora → Xikely → **Noe** |
...
| `256e69d` | LS API 集成（5 endpoint + LemonSqueezyClient.js），token 移到 `~/.noe-panel/` 0o600 |
...
✅ licenseKeys     Ed25519 私钥（~/.noe-panel-keys/）
✅ githubToken     gh CLI 已登录 BB20260410（fallback 查 ~/.config/gh/hosts.yml）
✅ lemonWebhook    secret 存 ~/.noe-panel/ls-webhook-secret.txt（fallback 路径）
...
- lemonWebhook: 查 ~/.noe-panel/ls-webhook-secret.txt（v2.1 secret 单文件路径）
...
✅ API token   ~/.noe-panel/lemonsqueezy-key.txt (0o600)，永不进 LLM 对话
✅ Product     1074614 'Noe Pro' status=published price=$19（顺带 store 还有 DuetCheck Pro 1065158，无关项目）
...
Cloudflare:
✅ Pages       https://noe.pages.dev/（官网，免费域名）
✅ Worker      https://noe-webhook.ilifelahepeq54.workers.dev
...
GitHub:
✅ Repo:   https://github.com/BB20260410/noe
...
含义：
- v1.5 Ed25519 私钥（~/.noe-panel-keys/）**当前未在 LS 主链路上**，仅作 fallback / 离线 / 自签发模式
...
| Apple Developer | 🟡 用户在做（另一项目「息刻」iOS App，App Store Connect tab 已开）| 跟 Noe 共享品牌 |
...
| 域名（noe.com / .ai）| ⏸️ 推迟 | 用 `noe.pages.dev` 免费域名占位，赚到钱再买 |
...
1. ⚠️ **LS test mode 真订单实测**（30 min，需操作 LS dashboard）
   ...
   - Worker 日志看：`wrangler tail noe-webhook`
   - panel 日志看：`tail -f ~/.noe-panel/logs/panel-*.log`
   - 如果链路有 bug，这里会暴露 — 比 v2.0 文档里"7 步手动"靠谱多了

2. ⚠️ **打 `v2.1.0-noe` tag**（5 min）
   - 4 个 v2.1 commit 还在 main 上没打 tag
   - 测试链路通了再打，免得测出 bug 还要回滚

### 中期 TODO（不卡发布）

1. **域名注册** noe.com / noe.ai — 用户决定赚到钱再买（暂用 `noe.pages.dev`）
...
├── package.json                           v2.0.0, productName="Noe"
├── README.md                              # Noe + 源码运行指南
├── EXTERNAL_REGISTRATIONS.md              5 个外部账号注册步骤
├── HANDOFF_v2.0_noe.md                ← 本文档
├── RELEASE_NOTES_v1.0/v1.5/v2.0.md        发布说明
├── src/
│   ├── license/LicenseManager.js          Ed25519 离线 license
│   ├── integrations/
│   │   └── LemonSqueezyClient.js          [v2.1] 全 LS REST API wrapper（token 从 ~/.noe-panel/ 读）
│   ├── server/routes/
...
├── website/                                [v2.1] Cloudflare Pages 静态站（noe.pages.dev）
│   ├── index.html                         landing page
│   ├── _headers / _redirects              CF Pages 配置
│   ├── favicon.svg
│   └── assets/logo.svg + logo-mono.svg
├── worker/                                 [v2.1] Cloudflare Worker（noe-webhook.ilifelahepeq54.workers.dev）
│   ├── wrangler.toml                      Worker 配置
│   └── src/index.js                       POST /webhooks/lemon + GET /api/license/verify + /health
├── public/
│   ├── index.html                         <title>Noe</title>
...
# 看商品化进度
curl http://127.0.0.1:51835/api/commercial/status | python3 -m json.tool

# 看 panel 日志
tail -f ~/.noe-panel/logs/panel-*.log

# GitHub 操作
gh repo view BB20260410/noe
gh release list -R BB20260410/noe
gh auth status
...
```
我接手 Noe 项目，刚读完 HANDOFF_v2.0_noe.md（含 v2.1 增量回填）。

panel pid: <lsof -ti tcp:51835>
GitHub repo: https://github.com/BB20260410/noe
最新 tag: v2.0.0-noe（v2.1 4 commit 在 main 还没打 tag）
...
🎁 接手后第一件事**永远不要做**：擅自重命名（用户已经改名 5 次，定的是 **Noe**，不要再改）。
