# Noe v2.x → v3.0 演进 Roadmap

> 文档版本：v1（2026-05-21 创建）
> 关联文档：`HANDOFF_v2.0_noe.md`、`RELEASE_NOTES_v2.0.md`
> 状态：草案，待你逐项拍板后执行

本 roadmap 基于 2026-05-21 对开源社区的横向调研（多模型 playground / 多 agent 编排 / 多 LLM 辩论 / SaaS boilerplate / Claude Code observability 五个方向），把"能借鉴的思路"映射为 Noe 自身的 5 条演进路径（D1–D5），按依赖关系排成可执行步骤。
...
### Step 4.4 测试套件（1 天）
- `test/local-mock.mjs` — 类似 `/tmp/noe-worker-test.mjs`，但通用化
...
- **Noe 官网首页底部加 "Powered by [我们自己开源的] lemonsqueezy-cf-worker-template"** → 反向引流

### 验收
- [ ] 独立 repo 可 `wrangler deploy` 跑起来
- [ ] README 一个完全陌生的开发者能照着 5 分钟跑通
- [ ] Noe worker/ 改成"使用同一个 template"（可选）
...
### Step 3.1 数据层（1 天）
- 检查 panel SQLite `~/.noe-panel/panel.db` 现有表结构
...
### Step 3.4 异常告警（半天）
- 加 `~/.noe-panel/cost-budget.json`（用户自填月预算）
- 超 80% 时 panel 顶部红条提醒
...
- 写到 `~/.noe-panel/detected-clis.json`
...
### Step 5.2 文档总装
- 改 `HANDOFF_v2.0_noe.md` → 拆成 `HANDOFF_v3.0.md` + `CHANGELOG_v2-to-v3.md`
- 更新官网 `website/` 首页：4 个核心模块改为 v3 卖点（带置信度的 debate、角色卡 squad、成本日历、CLI 接管）

### Step 5.3 release
```bash
git tag v3.0.0-noe
gh release create v3.0.0-noe --notes-file CHANGELOG_v2-to-v3.md
```
