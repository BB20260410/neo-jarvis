...
4. 脚本自动跳到 https://github.com/new 帮你创建 `noe` repo
5. 引导你生成 PAT（scope: repo）→ 你粘贴 token 到终端
6. 脚本存 token 到 `~/.noe-panel/github-token.json` (0o600)

### 后续：发 v1.0 Release

```bash
export GH_TOKEN=$(node -p "require(require('os').homedir()+'/.noe-panel/github-token.json').token")
...
- 验证：访问 https://github.com/<username>/noe/releases/tag/v1.0.0
...
5. 回 panel：`curl -X POST http://localhost:51835/api/webhooks/config -H 'Content-Type: application/json' -d '{"provider":"lemon","secret":"<刚才的 secret>"}'`
   - 注意 secret **不要发到对话里**，从 Apple Notes / 1Password 取

**f. 测试**
1. LS dashboard → 你的产品 → Test mode 下单
2. panel 端：`tail -f ~/.noe-panel/logs/panel-*.log` 看 webhook 是否签发 license
3. 看 `~/.noe-panel/licenses-issued.jsonl` 验签发记录
...
4. 存到 panel：`echo '{"token":"..."}' > ~/.noe-panel/keygen.json && chmod 600 ~/.noe-panel/keygen.json`
...
curl -X POST http://localhost:51835/api/license/activate \
  -H 'Content-Type: application/json' \
  -d "{\"license\":\"$LIC\"}"

# 3. 验证 tier 升 pro
curl http://localhost:51835/api/license/status | jq

# 4. 创建一个 squad 房（free 时会被拒，pro 时通过）
curl -X POST http://localhost:51835/api/rooms \
  -H 'Content-Type: application/json' \
...
curl -X POST http://localhost:51835/api/webhooks/lemon \
  -H "Content-Type: application/json" \
  -H "X-Signature: $SIG" \
  -d "$BODY"
