...
All notable changes to Noe will be documented in this file.
...
### Added
- 飞轮 stuck 根因修复(2026-07-03)：A1 `NOE_SELFEVO_TYPEERR_DETAIL`(type_error 错误详情喂 implementer,治盲猜)、A2 `NOE_SELFEVO_REPAIR_HINTS`(verify 失败原因回灌 self_repair,治 359 次盲重试)、B `NOE_SELFEVO_FAILFAST`(同因连败提前收口+stuckDrop 终态戳,治 139 尸体 cycle)三 flag 默认 OFF；C self_repair 补 typeErrorVerify 锚(堵假成功旁路)；D runtime-verify 报告持久化 failedTests 挂测名单。上轮交接假设 H1 被 DB 证据证伪,详见 docs/HANDOFF_2026-07-03_飞轮stuck根因修复.md。
- CE12 P0 文档编写：新增 `NOE_CE12_P0_DOCS_CANONICAL.md`、`NOE_CE12_P0_OPERATIONS_MANUAL.md`、`NOE_CE12_P0_HANDOFF.md` 和 `NOE_CE12_P0_DOCS_VERIFY.mjs`，把使用说明、维护说明、已知限制和交接信息收敛到 CE12 当前事实源。
- CE12 P0 文档验证入口：新增 `npm run verify:p0:docs`，校验 README、CHANGELOG、交接入口、证据索引、API 端点、Brain UI 锚点和“完整 Jarvis 产品未完成”口径。
- 阶段 9 文档编写：建立了 `NOE_PHASE9_DOCS_CANONICAL.md` 作为 Noe / BaiLongma 融合工作的单一权威文档，涵盖了使用说明、维护指南、已知限制和交接信息。同时新增了 `NOE_PHASE9_DOCS_VERIFY.mjs` 作为该文档的自动化验证门。
- 阶段 9 监督纠偏：新增 `NOE_STAGE9_SUPERVISOR_CORRECTION.md`，并把“最多讨论 3 轮、Claude 不可用时 GPT/Codex + Gemini 推进、不要因旧 CE05 返工文字回退”写入 canonical 文档、README、交接入口和验证门。
- **自我完善回流转正（六能力最后一块）**：软回流 lesson-pin（`NOE_LESSON_PIN` 把 learning_lesson/surprise_lesson 常驻注入每轮 chat 上下文，治“产而不用”——347 产仅 1 召回，`caf0cf2`）。加上硬回流 SelfDirectionSeed 连环修复（见 Fixed/Changed），飞轮现能持续自主反思改进方向 → 真改代码逻辑 → 真落地，修复后连续 12 done/0 dropped。详见 `docs/HANDOFF_2026-07-01_自我完善回流转正.md`。

### Changed
- README、`上下文交接.md`、`任务交接.md` 已改指向 CE12 P0 文档事实源，旧 `NOE_PHASE9_DOCS_CANONICAL.md` 仅保留为历史阶段文档，避免继续引用 CE12 前的过期结论。
- `NOE_CE12_P0_EVIDENCE_INDEX.md` 新增 CE09 文档切片，记录文档事实源、操作手册、交接文件和 `verify:p0:docs` 验证入口。
- 历史 Phase9 入口 `NOE_PHASE9_DOCS_CANONICAL.md` 已降级为历史参考，不再作为 CE12 P0 的当前事实源。
- `上下文交接.md` 与 `任务交接.md` 顶部入口已更新为 CE12 P0，避免后续执行者误回 `/Users/hxx/Desktop/00_项目/05_Claude可视化面板` 或旧 Phase9 口径。
- 移除了冗余文档 (`工作区入口.md`、`NOE_NEXT_EXECUTOR_HANDOFF.md`、`NOE_OPERATIONS_MANUAL.md`、`NOE_PHASE9_DOCUMENTATION.md`、`NOE_PHASE9_VERIFY.mjs`)，以确保文档集合的单一性和一致性。
- **自主方向纠偏**：SelfDirectionSeed 的 DIRECTION_SYSTEM prompt 从“首选补测试(test_only)”反转为“首选小 logic_changed（真改逻辑：加边界/空值检查、抽小纯函数、修小 bug）”，治自我完善退化成只刷测试覆盖率；`NOE_SELF_DIRECTION_COOLDOWN_MS` 从 6h 调至 1h（兼顾 half-open 试探间隔与失败喘息）。(`ace9dd7`)

### Fixed
- **SelfDirectionSeed 熔断永久死锁**：连续 3 dropped 触发 reward hacking 熔断后不再立新 goal → consecutiveNeutral 永≥3 → 永不恢复（实测卡 37h）。改用 CircuitBreaker half-open：cooldown 过后放行一次试探，成功则计数重置、失败则再熔断（受 `NOE_SELF_DIRECTION_COOLDOWN_MS` 门控，默认 0 逐字现状零回归）。(`5752030`)
- **自主方向 implement 编造文件路径致 apply preflight blocked**：self_directed 方向的 objective 只有模块名、无 src/ 完整路径 → `readTargetFileContext` 正则匹配不到 → minimax 猜路径（编造 `src/noe-tool-router.js` 等）→ `patch_replace_file_missing` → dropped。修：真实 `meta.targetFile`（完整路径）透传整条链（Trigger→executor→spawnImplementer），让 implementer 据真实文件出准 patch。修复后 file_missing dropped 归零（连续 12 done）。(`a6dbd22`)
...
- **C3 Codebase Index 上限集中**:FTS/vector/snapshot/code-context 上限并入 `src/agents/codebaseLimits.js`,支持 `~/.noe-panel/codebase-limits.json` 覆盖。(`e6d2a40`, `462e172`)
...
- **依赖漏洞清理**(`npm audit prod+dev = 0 漏洞`):见 Fixed 的 tmp ^0.2.6。注:1 个 high 是 Electron 37 CVE,**桌面端实际威胁面低**(`contextIsolation: true` + 仅加载可信 `localhost:51835`,CVE 多需加载不受信任 web 内容才可利用),server 模式不用 Electron 完全不受影响;升级被 better-sqlite3 12.10 上限约束,等上游适配。
...
[Unreleased]: https://github.com/BB20260410/noe/compare/v2.1.0...HEAD
[2.1.0]: https://github.com/BB20260410/noe/compare/v2.0.0...v2.1.0
[2.0.0]: https://github.com/BB20260410/noe/releases/tag/v2.0.0
