...
const PRIV_KEY_PATH = path.join(os.homedir(), '.noe-panel-keys', 'panel-license-private-key.pem');
if (!fs.existsSync(PRIV_KEY_PATH)) {
  console.error(`❌ 私钥不存在: ${PRIV_KEY_PATH}`);
  console.error('   先跑: openssl genpkey -algorithm ed25519 -out ~/.noe-panel-keys/panel-license-private-key.pem');
  process.exit(1);
}
...
console.log('');
console.log('买家激活:');
console.log(`  curl -X POST http://localhost:51835/api/license/activate -H 'Content-Type: application/json' -d '{"license":"${licenseStr}"}'`);
