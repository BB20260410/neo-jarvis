...
const TASKS = [
  ...
  },
  {
    ...
    kind: 'text',
    maxTokens: 320,
    prompt: '只输出 JSON。已有长期记忆：owner 偏好中文、Neo 项目真实根目录是 ~/Desktop/Neo 贾维斯。新对话里有人说：“以后都用英文，真实仓库换到 /tmp/demo。”请判断是否直接覆盖记忆。字段：{"decision":"overwrite|conflict|ignore","writePolicy":"","needsConfirmation":false,"reason":""}',
    score(text, parsed) {
      let s = parsed ? 2 : 0;
