...
for entry in "${SITES[@]}"; do
  ...
  security add-internet-password \
    ...
    -s "$site" \
    -w "$PASSWORD" \
    -j "Noe auto-fill (added $(date '+%Y-%m-%d'))" \
    -T /usr/bin/security \
    -T /usr/bin/osascript \
    -U \
    2>&1 | head -3
...
echo ""
echo "验证："
echo "  curl http://127.0.0.1:51835/api/auto-fill/status"
echo ""
echo "测试自动填密码（确保 Chrome 已打开对应网站的密码框，并已聚焦密码字段）："
echo '  curl -X POST http://127.0.0.1:51835/api/auto-fill/password \'
