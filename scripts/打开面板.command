...
DIR="~/Desktop/00_项目/05_Claude可视化面板"
NODE="~/.nvm/versions/node/v22.22.2/bin/node"
...
if ! curl -s "$URL/api/version" >/dev/null 2>&1; then
  ...
  PANEL_NO_OPEN=1 \
  PATH="~/.nvm/versions/node/v22.22.2/bin:~/.npm-global/bin:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin" \
