#!/usr/bin/env node
// dist-signed.mjs — 读 ~/.noe-panel/release-config.json 喂 env-var 给 electron-builder
...
//
// 流程:
//   1) 读 ~/.noe-panel/release-config.json(缺则告知字段)
...
function loadConfig() {
  if (!existsSync(CONFIG_PATH)) {
    fatal(`未找到 ${CONFIG_PATH}\n请先创建,字段:\n  ${REQUIRED.join(', ')} (+可选 notaryTeamId,缺省=teamId)\n` +
          `权限建议:mkdir -p ~/.noe-panel && chmod 700 ~/.noe-panel,文件 chmod 600。`);
  }
...
run('electron-builder --mac dmg (sign + notarize)', 'npx', ['electron-builder', '--mac', 'dmg', '--prepackaged', 'out/mac-arm64/Noe.app'], { env });
...
try {
  ...
  for (const f of fs.readdirSync(join(ROOT, 'out'))) {
    if (f.endsWith('.dmg') && f.startsWith('Noe')) dmgs.push(join(ROOT, 'out', f));
  }
} catch {}
if (!dmgs.length) fatal('未找到产出的 .dmg(out/Noe-*.dmg)');
const dmg = dmgs[0];
console.log(`\n📦 .dmg 产出:${dmg}`);
