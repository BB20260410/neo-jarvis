...
function discoverInstalledLaunchdLabels() {
  const labels = [];
  const agentsDir = join(homedir(), 'Library', 'LaunchAgents');
  for (const name of ['com.noe.panel.plist', 'com.hxx.noe.panel51835.plist', 'com.hxx.noe.panel.plist']) {
    const file = join(agentsDir, name);
    if (!existsSync(file)) continue;
    try {
      const text = readFileSync(file, 'utf8');
      const label = parseLaunchdPlistLabel(text);
      if (label) labels.push(label);
    } catch { /* ignore unreadable plist */ }
  }
  return [...new Set(labels)];
}

const _installedLaunchdLabels = discoverInstalledLaunchdLabels();
const _launchdResolved = resolveNoeLaunchdLabel({
  env: process.env,
  installedLabels: _installedLaunchdLabels,
});
const LAUNCHD_LABEL = _launchdResolved.label || NOE_CANONICAL_LAUNCHD_LABEL;
const LAUNCHD_LABEL_META = {
  ..._launchdResolved,
  installedLabels: _installedLaunchdLabels,
  alignment: buildLaunchdAlignmentReport({
    resolved: _launchdResolved,
    installedLabel: _installedLaunchdLabels[0] || null,
  }),
};
...
function launchdDisabled() {
  const uid = typeof process.getuid === 'function' ? process.getuid() : run('id', ['-u']).stdout.trim();
  const res = run('launchctl', ['print-disabled', `gui/${uid}`]);
  if (res.status !== 0) return null;
  return isLaunchdLabelDisabledInPrint(res.stdout || '', LAUNCHD_LABEL);
}

/**
 * Enable + bootstrap when service is disabled/unloaded, then kickstart.
 * Caller must ensure no unmanaged dual writer on live port before bootstrap.
 */
function ensureLaunchdSupervisedAndRestart() {
  const uid = typeof process.getuid === 'function' ? process.getuid() : run('id', ['-u']).stdout.trim();
  const plistPath = launchdPlistPath();
  const disabled = launchdDisabled() === true;
  ...
  const plan = planLaunchdSupervision({
    ...
    disabled,
    loaded,
    launchctlHasLabel: loaded,
    unmanagedLivePanel: false, // releasePort already handled by restart path when needed
  });
  const stepsRun = [];
  for (const step of plan.steps) {
    if (step.action === 'stop_unmanaged_panel') continue;
    if (!step.argv?.length) continue;
    const res = run('launchctl', step.argv);
    stepsRun.push({
      action: step.action,
      argv: step.argv,
      status: res.status,
  ...
  // Always kickstart -k as final ensure
  ...
  stepsRun.push({
    ...
    status: kick.status,
    stdout: (kick.stdout || '').trim().slice(0, 500),
    stderr: (kick.stderr || '').trim().slice(0, 500),
  });
  return {
    target,
    plan,
    stepsRun,
    status: kick.status,
    stdout: (kick.stdout || '').trim(),
    stderr: (kick.stderr || '').trim(),
  };
}

function restartLaunchd() {
  return ensureLaunchdSupervisedAndRestart();
}
