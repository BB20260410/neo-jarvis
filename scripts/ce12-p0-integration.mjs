  ...
  return new Promise((resolvePort, reject) => {
    ...
    srv.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        srv.close(() => reject(new Error('unable_to_resolve_integration_port')));
        return;
      }
...
async function main() {
  ...
  // 不使用该隔离 HOME 的默认 panel.db：非 live 端口会正确将默认库
  // 重写到 isolation/ 下，导致 harness 与 server 读不同路径。
  const dbPath = join(panelDir, 'ce12-panel.db');
  ...
  try {
    ...
    track('integration exception', false, { error: message });
    finalExtra = {
      ...finalExtra,
      error: stack || message,
    };
