...
beforeEach(() => {
  close();
  tmp = mkdtempSync(join(tmpdir(), 'noe-agent-runs-'));
...
describe('AgentRunStore', () => {
  ...
  it('countByStatus returns exact counts without list limit', () => {
    const store = new AgentRunStore({ logger: null });
    for (let i = 0; i < 3; i++) {
      store.create({ id: `run-c-${i}`, status: 'running', sessionId: 's' });
    }
    store.create({ id: 'run-c-done', status: 'succeeded', sessionId: 's', finishedAt: Date.now() });
    expect(store.countByStatus('running')).toBe(3);
    expect(store.countByStatus('succeeded')).toBe(1);
  });

  it('recoverStaleRunningRuns marks idle running as failed and spares recent running', () => {
    const store = new AgentRunStore({ logger: null });
    const now = Date.now();
    ...
    // Force updated_at into the past (create stamps now)
    store.db().prepare('UPDATE agent_runs SET updated_at = ? WHERE id = ?')
      .run(now - 2 * 60 * 60 * 1000, stale.id);
    ...
    const r = store.recoverStaleRunningRuns({
      now,
      olderThanMs: 30 * 60 * 1000,
      limit: 100,
    });
    expect(r.recovered).toBe(1);
    expect(store.get(stale.id).status).toBe('failed');
    expect(store.get(stale.id).finishedAt).toBeTruthy();
    expect(String(store.get(stale.id).error || '')).toMatch(/stale_running/);
    expect(store.get(fresh.id).status).toBe('running');
    expect(store.get(fresh.id).finishedAt).toBeFalsy();
  });
});
