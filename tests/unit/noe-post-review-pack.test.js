...
import {
  NOE_OPTIONAL_POST_REVIEW_MODELS,
...
describe('NoePostReviewPack', () => {
  it('builds a complete redacted review_work evidence pack for non-writer reviewers', () => {
    ...
    // P3 摘 xiaomi：默认 optional 集为空，reviewer 阵容不再出现 100% 不可用的 xiaomi。
    expect(pack.postReviewPlan.reviewers.some((r) => r.model === 'xiaomi')).toBe(false);
...
describe('P3 复核复活：xiaomi 摘除', () => {
  it('默认 optional reviewer 集为空（xiaomi 100% 不可用、303 次白占审计已摘）', () => {
    expect([...NOE_OPTIONAL_POST_REVIEW_MODELS]).toEqual([]);
  });
});
