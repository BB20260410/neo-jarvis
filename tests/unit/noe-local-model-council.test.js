...
describe('assertLocalCouncilLedgerSafe 统一走 NoeContextScrubber 单源（P0 单源化）', () => {
  it('识别 scrubber 扩展模式：github token', () => {
    const r = assertLocalCouncilLedgerSafe({ finalAnswer: `token ${'ghp_' + 'a1B2c3D4'.repeat(5)}` });
    expect(r.ok).toBe(false);
    expect(r.errors).toContain('ledger_contains_secret_like_text');
  });
});
