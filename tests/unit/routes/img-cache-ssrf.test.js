...
import { isPrivateIp, assertPublicUrl, createPinnedLookup } from '../../../src/server/routes/img-cache.js';
...
describe('assertPublicUrl', () => {
  ...
  it('拒非常规端口', async () => {
    ...
    await expect(assertPublicUrl('http://example.com:51835/')).rejects.toThrow();
  });
  ...
  it('为已校验地址创建绑定 DNS 结果的 lookup（fail-closed：mismatch 不回退裸 DNS）', async () => {
    const lookup = createPinnedLookup('example.test', [{ address: '8.8.8.8', family: 4 }]);
    // 匹配 host → 返回已校验公网 IP
    await expect(new Promise((resolve, reject) => lookup('example.test', {}, (err, address, family) => (err ? reject(err) : resolve({ address, family })))))
      .resolves.toEqual({ address: '8.8.8.8', family: 4 });
    // mismatch host → fail-closed 报错，绝不回退裸 DNS（否则 pin 形同虚设，可被 rebind 绕过）
    await expect(new Promise((resolve, reject) => lookup('other.test', {}, (err, address, family) => (err ? reject(err) : resolve({ address, family })))))
      .rejects.toThrow(/mismatch|blocked/);
  });
});
