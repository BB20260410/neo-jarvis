...
describe('knowledge evidence routes', () => {
  ...
  it('registers a read-only LLM Wiki search endpoint', async () => {
    const calls = [];
    const { app, routes } = makeApp();
    registerKnowledgeRoutes(app, {
      knowledgeStore,
      llmWikiRoot: '/tmp/wiki',
      wikiSearch: async (args) => {
        calls.push(args);
        return { ok: true, query: args.query, hits: [{ title: 'Karpathy', file: 'wiki/k.md', score: 3 }] };
      },
    });

    const res = makeRes();
    await findRoute(routes, 'get', '/api/knowledge/llm-wiki/search')
      .handlers[1]({ query: { q: 'Karpathy wiki', topK: '2' } }, res);

    expect(calls[0]).toEqual({ root: '/tmp/wiki', query: 'Karpathy wiki', topK: 2 });
    expect(res.payload).toEqual({ ok: true, query: 'Karpathy wiki', hits: [{ title: 'Karpathy', file: 'wiki/k.md', score: 3 }] });
  });
