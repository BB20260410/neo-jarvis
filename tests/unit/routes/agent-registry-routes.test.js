...
describe('agent registry routes', () => {
  ...
  it('registers an owner-gated changed-files endpoint with safe cwd resolution', async () => {
    ...
    const res = makeRes();
    await route.handlers[1]({ query: { cwd: '~/project' } }, res);
  ...
  it('returns a non-fatal empty changed-files preview when git status is unavailable', async () => {
    const { app, routes } = makeApp();
    registerAgentRegistryRoutes(app, {
      skillStore: fakeSkillStore,
      safeResolveFsPath: () => '/safe/non-git',
      gitStatusProvider() {
    ...
    await route.handlers[1]({ query: { cwd: '/tmp/non-git' } }, res);
  ...
  it('registers an owner-gated codebase-map endpoint with safe cwd resolution', async () => {
    ...
    try {
      ...
      const res = makeRes();
      await route.handlers[1]({ query: { cwd: '~/project', q: 'planner context', limit: '8' } }, res);
