...
// D 可观测修复(2026-07-03)：挂测「名字」此前读完即弃（vitest 中间 JSON 被 unlink），报告只有 numFailedTests
//   计数——实证 119/200 报告恰 1 挂测把 baseline/verify 拖死（逻辑门 baseline_not_green 全拒），却无法归因是哪个测试。
//   报告持久化 failedTests（fullName ≤10 条、每条截断），下次 flake 直接可归因。
describe('D 报告持久化挂测名单', () => {
  const failingReport = {
    numTotalTests: 100, numPassedTests: 99, numFailedTests: 1,
    testResults: [
      {
        name: '/repo/tests/unit/flaky.test.js',
        status: 'failed',
        assertionResults: [
          { status: 'passed', fullName: 'suite > ok case' },
          { status: 'failed', fullName: 'suite > flaky case that breaks baseline' },
        ],
      },
      { name: '/repo/tests/unit/good.test.js', status: 'passed', assertionResults: [{ status: 'passed', fullName: 'g > fine' }] },
    ],
  };

  it('挂测时返回值与落盘报告带 failedTests 名单', async () => {
    const r = await mk({ exitCode: 1, report: failingReport })({ root });
    expect(r.ok).toBe(false);
    expect(r.failedTests).toEqual(['suite > flaky case that breaks baseline']);
    // 落盘报告同样带名单（事后审计用）
    const { readFileSync } = await import('node:fs');
    const saved = JSON.parse(readFileSync(join(root, r.reportRef), 'utf8'));
    expect(saved.failedTests).toEqual(['suite > flaky case that breaks baseline']);
  });

  it('名单上限 10 条 + 每条截断（防报告膨胀）', async () => {
    const many = {
      numTotalTests: 100, numFailedTests: 15,
      testResults: [{
        name: '/repo/t.test.js', status: 'failed',
        assertionResults: Array.from({ length: 15 }, (_, i) => ({ status: 'failed', fullName: `case ${i} ${'x'.repeat(400)}` })),
      }],
    };
    const r = await mk({ exitCode: 1, report: many })({ root });
    expect(r.failedTests.length).toBe(10);
    expect(r.failedTests[0].length).toBeLessThanOrEqual(300);
  });

  it('全绿时 failedTests 为空数组（零噪音）', async () => {
    const r = await mk()({ root });
    expect(r.ok).toBe(true);
    expect(r.failedTests).toEqual([]);
  });

  it('报告缺 testResults（形状漂移）→ failedTests 空数组不崩（fail-open 只影响可观测）', async () => {
    const r = await mk({ exitCode: 1, report: { numTotalTests: 100, numFailedTests: 1 } })({ root });
    expect(r.ok).toBe(false);
    expect(r.failedTests).toEqual([]);
  });
});
