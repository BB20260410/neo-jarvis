...
beforeEach(() => {
  close();
  tmp = mkdtempSync(join(tmpdir(), 'noe-agent-verifier-'));
...
describe('AgentRunVerificationExecutor', () => {
  ...
  it('legacy ownerTrust=default defers explicit approval-gated file changes and resumes them after approval', async () => {
    execFileSync('git', ['init'], { cwd: tmp, stdio: 'ignore' });
    const store = new AgentRunStore({ logger: null });
...
describe('buildVerificationSpawnEnv（验证子进程 env 消毒）', () => {
  it('剥离 API key/token/注入类变量，保留 PATH/HOME 等安全键', () => {
    const env = buildVerificationSpawnEnv({
      PATH: '/usr/bin:/bin',
      HOME: '/Users/someone',
      LANG: 'zh_CN.UTF-8',
      MINIMAX_API_KEY: 'fake-key-for-test-000000000000',
      OPENAI_API_KEY: 'fake-key-2',
      NOE_OWNER_TOKEN: 'fake-token',
    ...
    expect(env.NOE_OWNER_TOKEN).toBeUndefined();
    expect(env.HTTP_PROXY).toBeUndefined();
    expect(env.NODE_OPTIONS).toBeUndefined();
    expect(env.DYLD_INSERT_LIBRARIES).toBeUndefined();
  });

  it('无参调用读 process.env 且不抛', () => {
    const env = buildVerificationSpawnEnv();
    expect(typeof env).toBe('object');
    expect(Object.keys(env).every((k) => !/API[_-]?KEY|SECRET|TOKEN|PASSWORD/i.test(k))).toBe(true);
  });
});
