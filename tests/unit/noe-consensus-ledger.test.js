...
describe('redactNoeConsensusText 统一走 NoeContextScrubber 单源（P0 单源化）', () => {
  it('覆盖 scrubber 扩展模式：ANTHROPIC key / JWT / 带连字符的 tp- key', () => {
    const jwt = ['eyJhbGciOiJIUzI1NiJ9', 'eyJzdWIiOiIxMjM0NTY3ODkwIn0', 'abcDEF123-_ghijkl'].join('.');
    const text = redactNoeConsensusText([
      'ANTHROPIC_API_KEY=fake-anthropic-key-000000',
      `bearer-less jwt ${jwt}`,
      'standalone tp-unit-test-redaction-key-11111111111111111111',
    ].join('\n'));
    expect(text).not.toContain('fake-anthropic-key-000000');
    expect(text).toContain('ANTHROPIC_API_KEY=[redacted]');
    expect(text).toContain('[redacted-jwt]');
    expect(text).not.toContain('tp-unit-test-redaction-key-11111111111111111111');
  });
});
