...
import { createNoeSocialDraft, markNoeSocialDraftExternalSideEffect, markNoeSocialDraftExternalSideEffectAttempt } from '../../src/runtime/NoeSocialPublishQueue.js';
import {
...
describe('NoeSocialFinalPublishExecutor', () => {
  ...
  // R2-P0 行为增强：draft 文件不可写时，现在在**点击前**的 attempt 预标记阶段就 fail closed（根本不点击、
  //   无外部副作用），比旧行为（点击后才发现落标失败、副作用已发生）更安全。原断言的「点击后 marker 失败」
  //   分支仍存在于代码中，但只读文件会更早在 attempt 阶段被拦。
  it('fails closed (before clicking) when the draft file cannot persist the attempt marker', async () => {
    const draftDir = mkdtempSync(join(tmpdir(), 'noe-social-final-publish-'));
    const calls = [];
    try {
      const draft = createDraft(draftDir);
      ...
      // 新行为：attempt 预标记写不进 → 点击前 fail closed，无外部副作用、osascript 未被调用。
      expect(out.blockers).toContain('final_publish_side_effect_attempt_marker_failed');
...
// R2-P0（2026-07-03）：不可逆副作用幂等洞——osascript 真点击发布在前、落标在后，若落标失败磁盘无痕迹，
//   上游重试穿透 :477 去重闸 → 同内容重复对外发布。正解：点击前预落 attempt 标记，去重闸认 attempt。
describe('R2-P0 社媒发布幂等：点击前预落 attempt 标记 + 去重闸认 attempt', () => {
  it('已有 externalSideEffectAttempted 的 draft 被去重闸阻断（不再点击 osascript）', async () => {
    const draftDir = mkdtempSync(join(tmpdir(), 'noe-social-attempt-'));
    const calls = [];
    try {
      const draft = createDraft(draftDir);
      markNoeSocialDraftExternalSideEffectAttempt({ dir: draftDir, id: draft.id });
      const out = await executeNoeSocialFinalPublish({
        draftDir,
        realExecute: true,
        args: { draftId: draft.id, platform: 'douyin', priorStageEvidence: chainStageEvidence(), browserState: { activeBrowser: { url: 'https://creator.douyin.com/', title: 'D' } } },
        deps: { spawn: () => { calls.push('spawned'); } },
      });
      expect(out.ok).toBe(false);
      expect(out.blockers).toContain('draft_already_has_external_side_effect');
      expect(calls).toEqual([]); // 绝不重复点击
    } finally { rmSync(draftDir, { recursive: true, force: true }); }
  });

  it('realExecute 点击前先把 attempt 标记落盘（即便点击后崩溃，磁盘已有痕迹挡住重试）', async () => {
    const draftDir = mkdtempSync(join(tmpdir(), 'noe-social-attempt2-'));
    try {
      const draft = createDraft(draftDir);
      const order = [];
      await executeNoeSocialFinalPublish({
        draftDir,
        realExecute: true,
        args: { draftId: draft.id, platform: 'douyin', priorStageEvidence: chainStageEvidence(), browserState: { activeBrowser: { url: 'https://creator.douyin.com/', title: 'D' } } },
        deps: {
          // 顺序断言：markAttempt 必须发生在 spawn 之前（点击前磁盘已有痕迹）。
          spawn: () => {
            order.push('spawn');
            return { stdout: { on() {} }, stderr: { on() {} }, on(ev, cb) { if (ev === 'close') queueMicrotask(() => cb(1, null)); } };
          },
          markAttempt: (opts) => { order.push('markAttempt'); return markNoeSocialDraftExternalSideEffectAttempt(opts); },
        },
      });
      expect(order[0]).toBe('markAttempt');
      expect(order).toContain('spawn');
    } finally { rmSync(draftDir, { recursive: true, force: true }); }
  });

  it('attempt 标记落盘失败 → 不点击 osascript（无痕迹下绝不发布，可安全重试）', async () => {
    const draftDir = mkdtempSync(join(tmpdir(), 'noe-social-attempt3-'));
    const calls = [];
    try {
      const draft = createDraft(draftDir);
      const out = await executeNoeSocialFinalPublish({
        ...
        deps: {
          ...
          markAttempt: () => ({ ok: false, error: 'disk_full' }),
        },
      });
      expect(out.ok).toBe(false);
      expect(out.blockers).toContain('final_publish_side_effect_attempt_marker_failed');
