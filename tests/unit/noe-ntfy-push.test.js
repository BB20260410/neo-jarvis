...
describe('createNtfyPusher', () => {
  it('JSON publish 到 base 根路径，body 含 topic/title/message（UTF-8 安全）', async () => {
    const sent = [];
    const p = createNtfyPusher({ topic: 'noe-owner', fetchImpl: async (url, init) => { sent.push({ url: String(url), body: JSON.parse(init.body) }); return {}; } });
    ...
    expect(sent[0].body.topic).toBe('noe-owner');
