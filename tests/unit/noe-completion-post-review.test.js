...
describe('P3 反橡皮章 + 送审证据自检', () => {
  it('本地 reviewer 系统提示词=怀疑者姿态（核对清单+证据缺失禁 approve），不再写“测试覆盖轻也放行”', async () => {
    let sys = '';
    const runner = makeLocalReviewerRunner('local-qwen', {
      structuredCall: async ({ messages }) => { sys = String(messages[0].content); return { ok: true, value: { decision: 'approve' } }; },
    });
    await runner({ prompt: 'p' });
    expect(sys).toContain('skeptic');
    expect(sys).toContain('abstain');
    expect(sys).not.toContain('even if test coverage is light');
  });

  it('runPostReview 送审前自检：tests 从 runtime 报告数字装配为非空（灭 m3 头号 reject 理由 tests-empty）', async () => {
    const { mkdtempSync, writeFileSync } = await import('node:fs');
    const { join } = await import('node:path');
    const { tmpdir } = await import('node:os');
    const root = mkdtempSync(join(tmpdir(), 'cpr-tests-'));
    writeFileSync(join(root, 'runtime.json'), JSON.stringify({ ok: true, numTotalTests: 8037, numFailedTests: 0 }));
    let packArgs = null;
    const adapter = makeNoeCompletionPostReview({
      root,
      buildPack: (args) => { packArgs = args; return { postReviewPlan: { reviewers: [] }, sha256: 'x' }; },
      runRound: async () => ({ postReview: { reviews: [] } }),
    });
    await adapter({ goal: 'g', runtimeReportRef: 'runtime.json', patchPlanRef: 'p.json', applyReportRef: 'a.json', rollbackRef: 'r.json', consensusLedgerRef: 'l.json', dir: 'out', rootAbs: root });
    expect(Array.isArray(packArgs.tests)).toBe(true);
    expect(packArgs.tests.length).toBeGreaterThan(0);
    expect(packArgs.tests.join(' ')).toContain('8037');
  });
});
