...
describe('noe-weak-targeted-local-drills', () => {
  ...
  afterEach(() => {
    ...
    dir = null;
    if (callerEnv) {
      if (callerEnv.port === undefined) delete process.env.PORT;
      else process.env.PORT = callerEnv.port;
      if (callerEnv.panelDbPath === undefined) delete process.env.PANEL_DB_PATH;
      else process.env.PANEL_DB_PATH = callerEnv.panelDbPath;
      callerEnv = null;
    }
  });
  ...
  it('runs targeted local drills with temp/fake inputs and temp-copy load-env isolation', async () => {
    callerEnv = {
      ...
      panelDbPath: process.env.PANEL_DB_PATH,
    };
    const callerPort = '51991';
    const callerPanelDbPath = join(tmpdir(), `noe-weak-targeted-caller-${process.pid}.db`);
    process.env.PORT = callerPort;
    process.env.PANEL_DB_PATH = callerPanelDbPath;
    ...
    expect(byFile.get('src/bootstrap/load-env.js')).toMatchObject({
      ...
      evidence: expect.objectContaining({
        ...
        isolationPolicyDependencyCopied: true,
    ...
    expect(process.env.PORT).toBe(callerPort);
    expect(process.env.PANEL_DB_PATH).toBe(callerPanelDbPath);
  });
});
