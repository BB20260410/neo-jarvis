...
//
// 历史：原版本用 curl 打本机 :51835 做 e2e 探针，CI 上 server 不在跑 → describe.skipIf 全跳过 = 假绿。
...
describe('safeResolveFsPath - 合法路径放行', () => {
  ...
  beforeEach(() => {
    ...
    TMP = mkdtempSync('/tmp/noe-sandbox-fuzz-');
  });
  afterEach(() => {
...
describe('safeResolveFsPathForWrite - 写入用变体', () => {
  let TMP;
  beforeEach(() => { TMP = mkdtempSync('/tmp/noe-sandbox-write-'); });
