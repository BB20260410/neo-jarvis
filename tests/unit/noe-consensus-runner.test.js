...
describe('Noe consensus runner', () => {
  it('runs injected online core three-model runners and produces a passing ledger', async () => {
    ...
    // P0 单源化后 policy 摘要改为 unified_source + vendor_key_shapes 措辞（覆盖面为旧列表严格超集）。
    expect(evidencePack).toContain('vendor_key_shapes');
    expect(evidencePack).toContain('unified_source: src/runtime/NoeContextScrubber.js');
