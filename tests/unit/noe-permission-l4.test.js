...
describe('L4 解枷锁：敏感目录访问', () => {
  ...
  it('ownerTrust=full：.ssh/.docker/.kube 直接 allow', () => {
    for (const p of ['~/.ssh/config', '~/.docker/config.json', '~/.kube/config']) {
  ...
  it('默认信任档现在是 full：.ssh 直接 allow', () => {
    expect(evalDir(dflt, '~/.ssh/config').decision).toBe('allow');
  });

  it('.aws/.gnupg 含密钥内容：full/default 当前都 allow，legacy default 才 deny', () => {
    for (const gov of [full, dflt]) {
      expect(evalDir(gov, '~/.aws/credentials').decision).toBe('allow');
      expect(evalDir(gov, '~/.gnupg/secring.gpg').decision).toBe('allow');
    }
    for (const gov of [legacy]) {
      expect(evalDir(gov, '~/.aws/credentials').decision).toBe('deny');
      expect(evalDir(gov, '~/.gnupg/secring.gpg').decision).toBe('deny');
    }
  });
});
